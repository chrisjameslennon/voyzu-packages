...
import { getStatusSemanticColor, StandardSettingsReadOnlyAlert } from "@voyzu-modules/all-modules/common/client";
...
export function FinancialDocumentTypeListContent({
  ...
  const refresh = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/financial-document-types${query}`);
      if (response.ok) {
