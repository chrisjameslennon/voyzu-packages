...
import { financeApiUrl } from "@voyzu-modules/all-modules/common/client";
import { useCallback, useEffect, useMemo, useRef, useState } from "react";

import type { FinancialPeriodResponseDto } from "@voyzu-modules/types/modules/financial-periods";
import type { FinancialYearResponseDto } from "@voyzu-modules/types/modules/financial-years";
import type { FinancialIntegrityResponseDto } from "@voyzu-modules/types/modules/company-reports";
import {
  Breadcrumbs,
  Button,
  Checkbox,
...
import localStyles from "./journal-entries-report.module.css";
...
interface FinancialIntegrityReportProps {
  pageTitle: string;
  initialData: FinancialIntegrityResponseDto | null;
...
export function FinancialIntegrityReport({
  pageTitle,
  initialData,
  initialFromDate,
  initialToDate,
  ...
  selectedCompanyId,
}: FinancialIntegrityReportProps) {
  const [data, setData] = useState<FinancialIntegrityResponseDto | null>(initialData);
  ...
  const [showSubledgerEntries, setShowSubledgerEntries] = useState(false);
  const [showSourceDocument, setShowSourceDocument] = useState(false);
  const isFirstRender = useRef(true);

  const fetchData = useCallback(async (companyId: number, rangeStart: string, rangeEnd: string, docType: string) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    ...
    void fetchData(selectedCompanyId, fromDate, toDate, documentTypeCode);
  }, [selectedCompanyId, fromDate, toDate, documentTypeCode, fetchData]);
  ...
  const documentTypeOptions = useMemo(
    () => [
      { value: "", label: "All document types", code: "" },
      ...(data?.documentTypes ?? []).map((type) => ({ value: type.code, label: type.name, code: type.code })),
    ],
    [data?.documentTypes],
  );
  ...
  const printableBase = "/finance/reports/financial-integrity/printable";
  const urlParams = new URLSearchParams({ fromDate, toDate });
  if (companyId) urlParams.set("companyId", String(companyId));
  if (documentTypeCode) urlParams.set("documentTypeCode", documentTypeCode);
  urlParams.set("showSubledgerEntries", String(showSubledgerEntries));
  urlParams.set("showSourceDocument", String(showSourceDocument));
  ...
  if (documentTypeCode) pdfParams.set("documentTypeCode", documentTypeCode);
  pdfParams.set("showSubledgerEntries", String(showSubledgerEntries));
  pdfParams.set("showSourceDocument", String(showSourceDocument));
  ...
  const optionItems: DropdownMenuItem[] = [
    {
      value: "show-subledger-entries",
      label: (
        <span className={optionStyles.checkboxOption}>
          <Checkbox checked={showSubledgerEntries} onChange={() => undefined} tabIndex={-1} />
          <span>Show subledger entries</span>
        </span>
      ),
      onSelect: () => setShowSubledgerEntries((checked) => !checked),
    },
    {
      value: "show-source-document",
      label: (
        <span className={optionStyles.checkboxOption}>
          <Checkbox checked={showSourceDocument} onChange={() => undefined} tabIndex={-1} />
          <span>Show source document</span>
        </span>
      ),
      onSelect: () => setShowSourceDocument((checked) => !checked),
    },
  ];
  ...
  return (
    <div className={layout.reportView}>
      ...
      <div className={layout.slotDocument}>
        ...
        {!loading && !data && (
          <div style={{ padding: "2rem", color: "var(--voyzu-color-text-muted)" }}>No data available. Select a company to view Financial Integrity.</div>
        )}
      </div>
    </div>
  );
