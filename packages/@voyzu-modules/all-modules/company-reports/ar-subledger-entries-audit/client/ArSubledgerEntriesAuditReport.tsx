...
import type { ArSubledgerEntriesAuditResponseDto } from "@voyzu-modules/types/modules/company-reports";
...
interface ArSubledgerEntriesAuditReportProps {
  pageTitle: string;
  initialData: ArSubledgerEntriesAuditResponseDto | null;
...
export function ArSubledgerEntriesAuditReport({
  pageTitle,
  initialData,
  initialFromDate,
  initialToDate,
  ...
  selectedCompanyId,
}: ArSubledgerEntriesAuditReportProps) {
  const [data, setData] = useState<ArSubledgerEntriesAuditResponseDto | null>(initialData);
  ...
  const fetchData = useCallback(async (companyId: number, rangeStart: string, rangeEnd: string) => {
    ...
    try {
      ...
      const res = await fetch(await financeApiUrl(`/reports/ar-subledger-entries-audit?${params.toString()}`));
      if (!res.ok) return;
      setData((await res.json()) as ArSubledgerEntriesAuditResponseDto);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const printableBase = "/finance/reports/ar-subledger-entries-audit/printable";
      ...
      <div className={layout.slotDocument}>
        ...
        {!loading && data && (
          <div className={layout.document} style={{ maxWidth: `${A4_LANDSCAPE_WIDTH_MM}mm` }}>
            <ArSubledgerEntriesAuditReportTemplate data={data} generatedAt={generatedAt} showSnapshotData={showSnapshotData} />
          </div>
        )}
        {!loading && !data && (
          <div style={{ padding: "2rem", color: "var(--voyzu-color-text-muted)" }}>No data available. Select a company to view the AR Subledger Entries.</div>
        )}
      </div>
    </div>
  );
