...
import type { ApSubledgerEntriesAuditResponseDto } from "@voyzu-modules/types/modules/company-reports";
...
import { ApSubledgerEntriesAuditReportTemplate } from "../templates/ApSubledgerEntriesAuditReportTemplate";
...
interface ApSubledgerEntriesAuditReportProps {
  pageTitle: string;
  initialData: ApSubledgerEntriesAuditResponseDto | null;
...
export function ApSubledgerEntriesAuditReport({
  pageTitle,
  initialData,
  initialFromDate,
  initialToDate,
  ...
  selectedCompanyId,
}: ApSubledgerEntriesAuditReportProps) {
  const [data, setData] = useState<ApSubledgerEntriesAuditResponseDto | null>(initialData);
  ...
  const fetchData = useCallback(async (companyId: number, rangeStart: string, rangeEnd: string) => {
    ...
    try {
      ...
      const res = await fetch(await financeApiUrl(`/reports/ap-subledger-entries-audit?${params.toString()}`));
      if (!res.ok) return;
      setData((await res.json()) as ApSubledgerEntriesAuditResponseDto);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const printableBase = "/finance/reports/ap-subledger-entries-audit/printable";
      ...
      <div className={layout.slotDocument}>
        ...
        {!loading && data && (
          <div className={layout.document} style={{ maxWidth: `${A4_LANDSCAPE_WIDTH_MM}mm` }}>
            <ApSubledgerEntriesAuditReportTemplate data={data} generatedAt={generatedAt} showSnapshotData={showSnapshotData} />
          </div>
        )}
        {!loading && !data && (
          <div style={{ padding: "2rem", color: "var(--voyzu-color-text-muted)" }}>No data available. Select a company to view the AP Subledger Entries.</div>
        )}
      </div>
    </div>
  );
