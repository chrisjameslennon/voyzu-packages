...
import type { JournalEntriesResponseDto } from "@voyzu-modules/types/modules/company-reports";
import {
  Breadcrumbs,
  Button,
  Checkbox,
...
import { JournalEntriesReportTemplate } from "../templates/JournalEntriesReportTemplate";
...
interface JournalEntriesReportProps {
  pageTitle: string;
  initialData: JournalEntriesResponseDto | null;
...
export function JournalEntriesReport({
  ...
  selectedCompanyId,
}: JournalEntriesReportProps) {
  const [data, setData] = useState<JournalEntriesResponseDto | null>(initialData);
  ...
  const printableBase = "/finance/reports/journal-entries/printable";
      ...
      <div className={layout.slotDocument}>
        ...
        {!loading && data && (
          <div className={layout.document} style={{ maxWidth: `${A4_LANDSCAPE_WIDTH_MM}mm` }}>
            <JournalEntriesReportTemplate data={data} generatedAt={generatedAt} showSnapshotData={showSnapshotData} />
          </div>
        )}
        {!loading && !data && (
          <div style={{ padding: "2rem", color: "var(--voyzu-color-text-muted)" }}>No data available. Select a company to view the Journal Entries.</div>
        )}
      </div>
    </div>
  );
