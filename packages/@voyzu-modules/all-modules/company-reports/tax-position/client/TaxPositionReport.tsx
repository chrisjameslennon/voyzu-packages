...
import type { TaxPositionResponseDto } from "@voyzu-modules/types/modules/company-reports";
...
interface TaxPositionReportProps {
  pageTitle: string;
  initialData: TaxPositionResponseDto | null;
  initialAsAtDate: string;
  selectedCompanyId: number | null;
}

export function TaxPositionReport({ pageTitle, initialData, initialAsAtDate, selectedCompanyId }: TaxPositionReportProps) {
  const [data, setData] = useState<TaxPositionResponseDto | null>(initialData);
  ...
  const printableBase = "/finance/reports/tax-position/printable";
    ...
    <div className={layout.reportView}>
      <header className={layout.reportHeader}>
        ...
        <div className={layout.slotTitle}>
          <div className={styles.titleIcon}><span className={`material-symbols-outlined ${styles.titleIconSymbol}`}>account_balance_wallet</span></div>
          <h1 className={`${typography.pageTitle} ${layout.pageTitleResponsive}`}>{pageTitle}</h1>
        </div>
        <div className={layout.slotToolbarLeft}>
          <div style={{ width: "200px" }}><DatePicker value={asAtDate} onChange={(date) => date && setAsAtDate(date)} clearable={false} /></div>
        </div>
      ...
      <div className={layout.slotDocument}>
        ...
        {!loading && data && (
          <div className={layout.document} style={{ maxWidth: "210mm" }}>
            <TaxPositionReportTemplate data={data} generatedAt={generatedAt} />
          </div>
        )}
        {!loading && !data && <div style={{ padding: "2rem", color: "var(--voyzu-color-text-muted)" }}>No data available. Select a company to view the tax position.</div>}
      </div>
    </div>
  );
}
