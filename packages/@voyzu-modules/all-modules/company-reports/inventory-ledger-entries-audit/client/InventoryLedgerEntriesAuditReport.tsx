...
import type { InventoryLedgerEntriesAuditResponseDto } from "@voyzu-modules/types/modules/company-reports";
...
interface InventoryLedgerEntriesAuditReportProps {
  pageTitle: string;
  initialData: InventoryLedgerEntriesAuditResponseDto | null;
...
export function InventoryLedgerEntriesAuditReport({
  pageTitle,
  initialData,
  initialFromDate,
  initialToDate,
  ...
  selectedCompanyId,
}: InventoryLedgerEntriesAuditReportProps) {
  const [data, setData] = useState<InventoryLedgerEntriesAuditResponseDto | null>(initialData);
  ...
  const fetchData = useCallback(async (companyId: number, rangeStart: string, rangeEnd: string) => {
    ...
    try {
      ...
      const res = await fetch(await financeApiUrl(`/reports/inventory-ledger-entries-audit?${params.toString()}`));
      if (!res.ok) return;
      setData((await res.json()) as InventoryLedgerEntriesAuditResponseDto);
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const printableBase = "/finance/reports/inventory-ledger-entries-audit/printable";
      ...
      <div className={layout.slotDocument}>
        ...
        {!loading && data && (
          <div className={layout.document} style={{ maxWidth: `${A4_LANDSCAPE_WIDTH_MM}mm` }}>
            <InventoryLedgerEntriesAuditReportTemplate data={data} generatedAt={generatedAt} showSnapshotData={showSnapshotData} />
          </div>
        )}
        {!loading && !data && (
          <div style={{ padding: "2rem", color: "var(--voyzu-color-text-muted)" }}>No data available. Select a company to view the Inventory Ledger Entries.</div>
        )}
      </div>
    </div>
  );
