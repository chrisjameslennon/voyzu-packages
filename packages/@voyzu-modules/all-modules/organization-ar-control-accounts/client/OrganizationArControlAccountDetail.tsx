...
import { UpdateGLAccount } from "@voyzu-modules/all-modules/common/control-accounts/domain/operation-policy";
import type { ControlAccountResponseDto } from "@voyzu-modules/types/modules/control-accounts";
...
import { OrganizationArControlAccountDetailsForm } from "./OrganizationArControlAccountDetailsForm";
  ...
  return (
    <div className={`${layoutStyles.detailView} ${layoutStyles.detailViewWithStatusRail}`}>
      <header className={layoutStyles.detailHeader}>
        ...
        <div className={layoutStyles.slotTitle}><div className={detailStyles.title}><div className={detailStyles.titleIcon}><span className={`material-symbols-outlined ${detailStyles.titleIconSymbol}`}>account_tree</span></div><h1 className={`${typography.pageTitle} ${layoutStyles.pageTitleResponsive}`}>{currentAccount.name}</h1></div></div>
        <div className={layoutStyles.slotActions}><div className={detailStyles.headerActions}><DetailBackButton fallbackHref={listPath} /><Button variant="primary" icon="save" disabled={saving || !hasChange || blockers.length > 0} onClick={() => { void save(); }}>{saving ? "Saving..." : "Save"}</Button></div></div>
      ...
      <aside className={layoutStyles.statusSection}>
        <div className={detailStyles.card}>
          {currentAccount.hasPostings ? <div className={detailStyles.fieldGroup}><Badge variant="soft" size="medium" customColors={getHasPostingsColor(true)}>HAS POSTINGS</Badge></div> : null}
          <div className={detailStyles.fieldGroup}><label className={typography.fieldLabel}>Companies with postings</label><p className={typography.bodyText}>{currentAccount.companiesWithPostings.length ? currentAccount.companiesWithPostings.join(", ") : "None"}</p></div>
        </div>
      ...
      <main className={layoutStyles.mainSection}><OrganizationArControlAccountDetailsForm account={currentAccount} glAccounts={selectableGlAccounts} glAccountId={glAccountId} disabled={!canSelectAlternative} onGlAccountChange={setGlAccountId} /></main>
