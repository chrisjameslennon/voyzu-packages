...
} from "@voyzu-modules/types/modules/financial-document-processing-engine/ledger-journal-reversal.response.dto";
import type {
  LedgerJournalJournalLineDto,
  LedgerJournalPostingDetailsDto,
} from "@voyzu-modules/types/modules/financial-document-processing-engine/ledger-journal.response.dto";
...
export async function processLedgerJournalReversal(
  ...
  return withTransaction(async (client) => {
    ...
    const header = await journalRepo.insert({
      ...
      company_id: txContext.data.company!.id,
