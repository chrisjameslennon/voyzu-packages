...
} from "@voyzu-modules/types/modules/financial-document-processing-engine/ar-receipt.response.dto";
...
async function getCompany(db: DbExecutor, code: string): Promise<CompanyRow | null> {
  return one(db, `SELECT id, code, name, country_code, base_currency_code, status FROM company WHERE code = $1`, [code], companyRow);
}
...
async function getCounterparty(db: DbExecutor, companyId: number, code: string): Promise<CounterpartyRow | null> {
  return one(db, `SELECT cp.*, c.currency_code AS country_currency_code FROM ar_counterparty cp JOIN country c ON c.code = cp.country_code WHERE cp.company_id = $1 AND cp.code = $2`, [companyId, code], counterpartyRow);
}
  ...
  const row = await one(db,
    code
      ...
      : `SELECT pc.code, bca.code AS bank_cash_control_account_code,
       ...
       JOIN bank_cash_control_account bca ON bca.company_id = pc.company_id AND bca.id = pc.bank_cash_control_account_id
       JOIN gl_account ga ON ga.company_id = bca.company_id AND ga.id = bca.gl_account_id
