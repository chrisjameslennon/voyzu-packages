...
import { getGlAccountTypeColor, getStatusSemanticColor } from "@voyzu-modules/all-modules/common/client";
import { Deactivate, Delete } from "@voyzu-modules/all-modules/common/gl-accounts/domain/operation-policy";
import { AddGlAccountModal, GL_ACCOUNT_CODE_PATTERN, GL_ACCOUNT_TYPE_OPTIONS } from "../../common/gl-accounts/client";
import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/navigation";

import type { GlAccountCategoryResponseDto } from "@voyzu-modules/types/modules/gl-account-categories";
import type { GlAccountCreateRequestDto, GlAccountResponseDto } from "@voyzu-modules/types/modules/gl-accounts";
import {
  Badge,
  Breadcrumbs,
  Button,
...
interface OrganizationGlAccountsListContentProps {
...
const columns: DataTableColumn<GlAccountResponseDto>[] = [
  ...
  },
  {
    ...
    render: (row) => row.category ? (
      <span className={listStyles.nameWithCode}>
        <span>{row.category.name}</span>
        <span className={listStyles.codeChip}>{row.category.code}</span>
      </span>
    ) : "-",
  },
  {
...
export function OrganizationGlAccountsListContent({
  accounts,
  categories,
}: OrganizationGlAccountsListContentProps) {
  ...
  const filtered = useMemo(() => {
    ...
    if (query) {
      result = result.filter((account) => (
        ...
        (account.category?.code ?? "").toLowerCase().includes(query) ||
        ...
        <div className={layoutStyles.slotTitle}>
          ...
          <div className={layoutStyles.slotTitleByline}>
            <p className={typography.headingByline}>
              General Ledger accounts for new and linked companies.
            </p>
          </div>
        </div>
        <div className={layoutStyles.slotActions}>
          <Button variant="primary" icon="add" className={layoutStyles.slotPrimaryAction} onClick={() => setIsAddOpen(true)}>
