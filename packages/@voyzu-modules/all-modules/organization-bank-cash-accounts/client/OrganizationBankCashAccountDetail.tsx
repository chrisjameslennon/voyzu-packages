...
import { getHasPostingsColor, getStatusSemanticColor } from "@voyzu-modules/all-modules/common/client";
import { ChangeCodeAvailability, ChangeTypeAvailability, Deactivate, Delete, UpdateGLAccount } from "@voyzu-modules/all-modules/common/bank-cash-accounts/domain/operation-policy";
import type { BankCashAccountPatchRequestDto, BankCashAccountResponseDto } from "@voyzu-modules/types/modules/bank-cash-accounts";
...
interface OrganizationBankCashAccountDetailProps {
      ...
      <header className={layoutStyles.detailHeader}>
        ...
        <div className={layoutStyles.slotActions}>
          <div className={detailStyles.headerActions}>
            ...
            <Button variant="secondary" icon="check_circle" disabled={account.status === "ACTIVE"} onClick={() => { void transitionStatus("activate"); }}>Activate</Button>
            <Button variant="secondary" icon="block" disabled={account.status === "INACTIVE" || deactivateBlockers.length > 0} onClick={() => { void transitionStatus("deactivate"); }}>Deactivate</Button>
            <Button variant="danger" icon="delete" disabled={deleteBlockers.length > 0} onClick={() => { void deleteAccount(); }} />
          </div>
        </div>
      ...
      <aside className={layoutStyles.statusSection}>
        <div className={detailStyles.card}>
          ...
          <div className={detailStyles.fieldGroup}>
            ...
            <p className={typography.bodyText}>{account.companiesWithPostings.length > 0 ? account.companiesWithPostings.join(", ") : "None"}</p>
          </div>
        </div>
        <div className={detailStyles.card}>
          <div className={detailStyles.fieldGroup}>
