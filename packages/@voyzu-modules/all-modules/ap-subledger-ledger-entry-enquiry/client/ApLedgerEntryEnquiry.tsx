...
import { CompanyPageTitleBadges, financeApiUrl, getDrCrColor, getStatusSemanticColor } from "@voyzu-modules/all-modules/common/client";
...
import localStyles from "./ap-ledger-entry-enquiry.module.css";
...
interface Props {
  entries: ApSubledgerEntryResponseDto[];
...
export function ApLedgerEntryEnquiry({ entries, financialYears, periods: initialPeriods, selectedYearCode: initialYearCode, fromDate: initialFromDate, toDate: initialToDate }: Props) {
  ...
  const exportItems = useMemo<DropdownMenuItem[]>(() => [
    { value: "selected", label: `Selected (${selectedRows.length})`, icon: "check_box", disabled: selectedRows.length === 0, onSelect: () => { void handleExport(selectedRows, "ap_ledger_entry_enquiry_selected"); } },
    { value: "current-view", label: `Current view (${filtered.length})`, icon: "visibility", disabled: filtered.length === 0, onSelect: () => { void handleExport(filtered, "ap_ledger_entry_enquiry_current_view"); } },
    { value: "full-dataset", label: `Full dataset (${rows.length})`, icon: "database", disabled: rows.length === 0, onSelect: () => { void handleExport(rows, "ap_ledger_entry_enquiry_full_dataset"); } },
      ...
      <header className={layout.reportHeader}>
        ...
        <div className={layout.slotTitle}>
          ...
          <div className={layout.slotTitleText}>
            <h1 className={`${typography.pageTitle} ${layout.pageTitleResponsive}`}>AP Ledger Entry Enquiry</h1>
        ...
        <div className={localStyles.filterToolbar}>
          ...
          <Input search containerClassName={localStyles.search} placeholder="Search AP entries..." value={search} onChange={(event) => { setSearch(event.target.value); setCurrentPage(1); }} />
        </div>
      </header>
