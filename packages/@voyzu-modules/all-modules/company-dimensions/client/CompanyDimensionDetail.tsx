...
import { CompanySettingsTitleBadges, getHasPostingsColor, getStatusSemanticColor } from "@voyzu-modules/all-modules/common/client";
import { ChangeCodeAvailability, ChangeValueNameAvailability, Deactivate, Delete, DeleteValue } from "@voyzu-modules/all-modules/common/dimensions/domain/operation-policy";
import type { DimensionPatchRequestDto, DimensionResponseDto, DimensionValueCreateRequestDto, DimensionValuePatchRequestDto, DimensionValueResponseDto, DimensionValueStatus } from "@voyzu-modules/types/modules/dimensions";
...
interface CompanyDimensionDetailProps {
      ...
      render: (row) => (
        <span className={listStyles.inlineGroup}>
          <Button
            ...
            disabled={readOnly || ChangeValueNameAvailability(row).length > 0}
          ...
          <Button
            ...
            disabled={readOnly || DeleteValue(row).length > 0}
      ...
      <header className={layoutStyles.detailHeader}>
        ...
        <div className={layoutStyles.slotActions}>
          <div className={detailStyles.headerActions}>
            ...
            <Button variant="secondary" icon="check_circle" disabled={readOnly || dimension.status === "ACTIVE"} onClick={() => { void transitionStatus("activate"); }}>Activate</Button>
            <Button variant="secondary" icon="block" disabled={readOnly || dimension.status === "INACTIVE" || Deactivate(dimension).length > 0} onClick={() => { void transitionStatus("deactivate"); }}>Deactivate</Button>
            <Button variant="danger" icon="delete" disabled={readOnly || Delete(dimension).length > 0} onClick={() => { void deleteDimension(); }} />
          </div>
        </div>
      ...
      <main className={layoutStyles.mainSection}>
        ...
        <section className={detailStyles.card}>
          <div className={detailStyles.cardHeader}>
            ...
            <Button variant="secondary" icon="add" disabled={readOnly} onClick={() => setIsAddValueOpen(true)}>
              Add Value
            </Button>
          </div>
          <DataTable
