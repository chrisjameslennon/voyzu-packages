...
import { CompanyPageTitleBadges, financeApiUrl, getHasPostingsColor, getStatusSemanticColor } from "@voyzu-modules/all-modules/common/client";
import { InventoryItemDetailsForm } from "@voyzu-modules/all-modules/common/inventory-items/client";
import { ChangeCodeAvailability, Deactivate } from "@voyzu-modules/all-modules/common/inventory-items/domain/operation-policy";
import { useRouter } from "next/navigation";
import { useState } from "react";

import type { InventoryCategoryResponseDto } from "@voyzu-modules/types/modules/inventory-categories";
import type { InventoryItemPatchRequestDto } from "@voyzu-modules/types/modules/inventory-items";
import type { InventoryItemResponseDto } from "@voyzu-modules/types/modules/inventory-items";
...
export function InventoryItemDetail({ item, categories, listPath = "/finance/inventory/items" }: { item: InventoryItemResponseDto; categories: InventoryCategoryResponseDto[]; listPath?: string }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      const response = await fetch(await financeApiUrl(`/inventory/items/${encodeURIComponent(currentItem.item_code)}`), { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
      if (!response.ok) {
  ...
  const transitionStatus = async (action: "activate" | "deactivate") => {
    setServerError("");
    const response = await fetch(await financeApiUrl(`/inventory/items/${encodeURIComponent(currentItem.item_code)}/${action}`), { method: "POST" });
    if (!response.ok) {
      const body = await response.json().catch(() => null) as { message?: string } | null;
      setServerError(body?.message ?? "An unexpected error occurred");
      return;
  ...
  const deleteItem = async () => {
    setServerError("");
    const response = await fetch(await financeApiUrl(`/inventory/items/${encodeURIComponent(currentItem.item_code)}`), { method: "DELETE" });
    if (!response.ok) {
      const body = await response.json().catch(() => null) as { message?: string } | null;
      setServerError(body?.message ?? "An unexpected error occurred");
      ...
      <aside className={layout.statusSection}>
        <div className={detailStyles.card}>
          <div className={detailStyles.fieldGroup}>
            ...
            {currentItem.hasPostings ? <Badge variant="soft" size="medium" customColors={getHasPostingsColor(true)}>HAS POSTINGS</Badge> : null}
          </div>
        </div>
        <AuditPanel
          id={currentItem.id}
