...
import { listFinancialYears } from "@voyzu-modules/core/financial-years/server";
import { listPeriods } from "@voyzu-modules/core/financial-years/server";

import { ApSubledgerEntriesAuditReport } from "../../client";
...
export async function ApSubledgerEntriesAuditReportPage({ surface }: ReportPageProps = {}) {
  ...
  const initialData = await getApSubledgerEntriesAudit(company.id, fromDate, toDate);

  if (surface?.unframed) {
    return (
      <ApSubledgerEntriesAuditReportTemplate
        data={initialData}
