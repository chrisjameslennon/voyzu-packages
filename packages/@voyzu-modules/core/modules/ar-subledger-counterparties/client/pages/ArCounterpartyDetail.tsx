...
import type { ArCounterpartyResponseDto } from "@voyzu-modules/core/types/modules/ar-subledger";
...
export function ArCounterpartyDetail({ counterparty }: { counterparty: ArCounterpartyResponseDto }) {
  ...
  return (
    <div className={`${layout.detailView} ${layout.detailViewWithStatusRail}`}>
      ...
      <main className={layout.mainSection}><section className={detailStyles.card}><h2 className={typography.sectionHeading}>AR Counterparty</h2><div className={detailStyles.formGrid}>{fields.map((field) => <label key={field.label} className={detailStyles.fieldGroup}><span className={typography.fieldLabel}>{field.label}</span><Input value={field.value} disabled /></label>)}</div></section></main>
    </div>
  );
}
