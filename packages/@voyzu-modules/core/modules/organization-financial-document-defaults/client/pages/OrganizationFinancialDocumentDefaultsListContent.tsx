...
import { getStatusSemanticColor, StandardSettingsReadOnlyAlert } from "@voyzu-modules/core/common/client";
...
export function FinancialDocumentDefaultsListContent({
  ...
  const refresh = async () => {
    ...
    setRefreshing(true);
    try {
      const query = apiScope === "template" ? "?scope=template" : "";
      const response = await fetch(`/api/financial-document-defaults${query}`);
      if (response.ok) {
