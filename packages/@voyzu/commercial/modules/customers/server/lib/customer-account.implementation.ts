...
export async function get({ party_id }: { party_id: number }): Promise<CustomerAccount | null> {
  const account = customerData.accounts.find(account => account.party_id === party_id);
...
export async function adjustCreditLimit({
  ...
  console.log("[voyzu] Customer credit limit adjustment", { party_id, amount });
}

export async function update({ party_id, changes }: Parameters<CustomerAccountMethods["update"]>[0]): Promise<void> {
  const account = customerData.accounts.find(account => account.party_id === party_id);
  if (!account) throw new Error("Customer account not found");
