"use client";
import { SearchableSelect, Input, Textarea, useFormValidation, required, maxLength, pattern } from "@voyzu/ui-components";
import detail from "@voyzu/ui-style/css-modules/detail.module.css";
import typography from "@voyzu/ui-style/css-modules/typography.module.css";
import { addressTypes, emptyAddress, type CustomerInput } from "../types/customer.dto";
...
export function CustomerFields({ value, onChange, pending, hasError, creating = false, categories, priceLists }: { categories: { code: string; name: string; status: string }[]; priceLists: { code: string; name: string; status: string }[]; value: CustomerInput; onChange: (value: CustomerInput) => void; pending: boolean; hasError: (key: string) => boolean; creating?: boolean }) {
  return <>
    ...
    <div className={detail.formGrid}>{([["categoryCode", "Customer Category", categories], ["priceListCode", "Customer Price List", priceLists]] as const).map(([key, label, rows]) => <div className={detail.fieldGroup} key={key}><span className={typography.fieldLabel}>{label}</span><SearchableSelect ariaLabel={label} value={value[key] ?? ""} clearable disabled={pending} options={rows.filter((row) => row.status === "ACTIVE" || row.code === value[key]).map((row) => ({ value: row.code, label: row.name }))} onChange={(code) => onChange({ ...value, [key]: code || null })} placeholder={"Select a " + label.toLowerCase()} />{!creating && value[key] && <a className={typography.link} href={"/commercial/customers/" + (key === "categoryCode" ? "customer-categories/" : "price-lists/") + encodeURIComponent(value[key]!)}>View {label.toLowerCase()}</a>}</div>)}</div>
    {!creating && <>
      {addressTypes.map((type) => {
        const address = value.addresses.find((row) => row.address_type === type) ?? emptyAddress(type);
        return <section className={detail.dividedStack} key={type}><h3 className={typography.sectionHeading}>{type === "PRIMARY" ? "Primary Address" : type === "SHIPPING" ? "Shipping Address" : "Postal Address"}</h3><div className={detail.formGrid}>{addressFields.map(([key, label]) => <div className={detail.fieldGroup} key={key}><label className={typography.fieldLabel} htmlFor={type + key}>{label}</label><Input id={type + key} disabled={pending} invalid={hasError(type + "." + key)} value={address[key]} placeholder={key === "country_code" ? "e.g. NZ" : undefined} onChange={(event) => { const next = { ...address, [key]: key === "country_code" ? event.target.value.toUpperCase() : event.target.value }; onChange({ ...value, addresses: addressTypes.map((role) => role === type ? next : value.addresses.find((row) => row.address_type === role) ?? emptyAddress(role)) }); }} /></div>)}</div></section>;
      })}
