...
export function ProductModal({ title, children, pending, onClose, onSubmit, submitLabel }: { title: string; children: ReactNode; pending: boolean; onClose: () => void; onSubmit: () => void; submitLabel: string }) {
 const ref = useRef<HTMLDialogElement>(null);
 useEffect(() => { ref.current?.showModal(); }, []);
 return <dialog ref={ref} className={modal.modal + " " + styles.standardDialog} aria-label={title} onCancel={(event) => { event.preventDefault(); if (!pending) onClose(); }}>
