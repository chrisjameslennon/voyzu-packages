...
async function requireAdmin(): Promise<void> {
  if (!(await capabilities.use("platform.identity").getCurrentIdentity({})).permissions.includes("users.manage")) {
...
export async function replaceUserOrganizationAccess(
  ...
  const user = await semanticData.get("user", normalizedCode);
