...
--           finance_organization_id matches parent FY
-- ============================================================
...
DECLARE
  v_fy_finance_organization_id BIGINT;
  v_fy_start      DATE;
  v_fy_end        DATE;
...
BEGIN
  ...
  SELECT finance_organization_id, start_date, end_date
    INTO v_fy_finance_organization_id, v_fy_start, v_fy_end
  FROM fiscal_year
  WHERE id = NEW.fiscal_year_id;

  IF v_fy_finance_organization_id IS NULL THEN
    RAISE EXCEPTION 'fiscal_year with id % not found', NEW.fiscal_year_id;
  END IF;

  IF NEW.finance_organization_id != v_fy_finance_organization_id THEN
    RAISE EXCEPTION 'fiscal_period.finance_organization_id (%) must match fiscal_year.finance_organization_id (%)',
      NEW.finance_organization_id, v_fy_finance_organization_id;
  END IF;
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    FROM fiscal_period
    WHERE finance_organization_id = NEW.finance_organization_id
