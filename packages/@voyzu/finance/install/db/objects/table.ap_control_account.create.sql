...
CREATE TABLE IF NOT EXISTS ap_control_account (
    ...
    CONSTRAINT pk_ap_control_account PRIMARY KEY (finance_organization_id, code),
    CONSTRAINT fk_ap_control_account_company FOREIGN KEY (finance_organization_id) REFERENCES finance_organization(id) ON DELETE CASCADE,
    CONSTRAINT fk_ap_control_account_gl_account FOREIGN KEY (finance_organization_id, gl_account_id) REFERENCES gl_account(finance_organization_id, id)
);
