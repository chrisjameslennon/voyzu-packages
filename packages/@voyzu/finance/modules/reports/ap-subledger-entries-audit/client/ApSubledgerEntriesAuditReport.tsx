...
import type { ApSubledgerEntriesAuditResponseDto } from "../types/ap-subledger-entries-audit.response.dto";
...
export function ApSubledgerEntriesAuditReport({
  ...
  const fetchData = useCallback(async (companyId: number, rangeStart: string, rangeEnd: string) => {
    ...
    try {
      ...
      const res = await fetch(await financeHttpApiUrl(`/reports/ap-subledger-entries-audit?${params.toString()}`));
      if (!res.ok) return;
