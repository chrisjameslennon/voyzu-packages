...
import type { ArSubledgerEntriesAuditResponseDto } from "../types/ar-subledger-entries-audit.response.dto";
...
export function ArSubledgerEntriesAuditReport({
  ...
  const fetchData = useCallback(async (companyId: number, rangeStart: string, rangeEnd: string) => {
    ...
    try {
      ...
      const res = await fetch(await financeHttpApiUrl(`/reports/ar-subledger-entries-audit?${params.toString()}`));
      if (!res.ok) return;
