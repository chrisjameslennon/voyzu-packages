...
} from "@voyzu/finance/types/modules/financial-document-types";
...
export function CompanyFinancialDocumentTypeDetail({
  ...
  const routePrefix = "/finance/integration";
  const tabs: TabDef[] = [
    {
      key: "posting",
      label: "Posting",
    ...
    <div className={`${layoutStyles.detailView} ${layoutStyles.detailViewWithStatusRail}`}>
      ...
      <aside className={layoutStyles.statusSection}>
        ...
        <AuditPanel
          ...
          auditHref={`/settings/audit?entityType=financial_document_type&entityCode=${encodeURIComponent(processor.code)}`}
