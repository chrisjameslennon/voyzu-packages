...
function validateCounterparty(input: ApBillRequestDto, data: ApBillDataValidationContext, errors: string[]): void {
  ...
  if (data.company && data.counterparty.finance_organization_id !== data.company.id) errors.push(`AP counterparty ${data.counterparty.code} does not belong to company ${data.company.code}`);
