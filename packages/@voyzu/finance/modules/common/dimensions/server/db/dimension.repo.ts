...
const COLUMNS: readonly string[] = [
  "id", "finance_organization_id", "code", "name", "status",
...
const COMPANIES_WITH_POSTINGS_SQL = `COALESCE(ARRAY(
  ...
  WHERE source_finance_organization.id = d.finance_organization_id
...
export class DimensionRepo {
  ...
  async get(companyId: number, code: string): Promise<DimensionRow | null> {
    const { rows } = await this.db.query(
      `${SELECT_WITH_DERIVED} WHERE d.finance_organization_id = $1 AND d.code = $2 AND d.status != 'DELETED'`,
      [companyId, code],
    );
  ...
  async listAll(companyId: number): Promise<DimensionRow[]> {
    const { rows } = await this.db.query(
      `${SELECT_WITH_DERIVED} WHERE d.finance_organization_id = $1 AND d.status != 'DELETED' ORDER BY d.code ASC`,
      [companyId],
    );
  ...
  async filter(companyId: number, filters: Filter[], options?: ListOptions): Promise<DimensionRow[]> {
    ...
    const fullWhere = shiftedWhere ? `WHERE d.finance_organization_id = $1 AND ${shiftedWhere.slice("WHERE ".length)} AND d.status != 'DELETED'` : `WHERE d.finance_organization_id = $1 AND d.status != 'DELETED'`;
  ...
  async search(companyId: number, phrase: string, options?: ListOptions): Promise<DimensionRow[]> {
    ...
    const whereSql = `WHERE d.finance_organization_id = $1 AND (${likeParts.join(" OR ")}) AND d.status != 'DELETED'`;
  ...
  async batchGet(companyId: number, codes: string[]): Promise<DimensionRow[]> {
    ...
    const { rows } = await this.db.query(
      `${SELECT_WITH_DERIVED} WHERE d.finance_organization_id = $1 AND d.code = ANY($2::text[]) AND d.status != 'DELETED' ORDER BY d.code ASC`,
      [companyId, codes],
    );
    return rows.map((r: Record<string, unknown>) => this.mapRow(r));
  }
