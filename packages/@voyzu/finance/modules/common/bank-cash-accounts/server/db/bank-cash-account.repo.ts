...
const COMPANIES_WITH_POSTINGS_SQL = `COALESCE(ARRAY(
           ...
           WHERE source_finance_organization.id = bca.finance_organization_id
...
const LINKED_BY_SQL = `COALESCE(ARRAY(
           ...
           WHERE fdd.finance_organization_id = bca.finance_organization_id
...
const SELECT_SQL = `
  ...
  FROM ${TABLE} bca
  LEFT JOIN gl_account ga ON ga.finance_organization_id = bca.finance_organization_id AND ga.id = bca.gl_account_id
`;
...
export class BankCashAccountRepo {
  ...
  async insert(row: InsertBankCashAccountRow): Promise<BankCashAccountRow> {
    const { rows } = await this.db.query(
      `INSERT INTO ${TABLE}
         (finance_organization_id, code, ledger, type, gl_account_id, bank_name, bank_branch_name, bank_account_identifier, cash_account_identifier, status,
  ...
  async get(companyId: number, code: string): Promise<BankCashAccountRow | null> {
    const { rows } = await this.db.query(
      `${SELECT_SQL} WHERE bca.finance_organization_id = $1 AND bca.code = $2`,
      [companyId, code],
    );
  ...
  async getById(companyId: number, id: number): Promise<BankCashAccountRow> {
    const { rows } = await this.db.query(
      `${SELECT_SQL} WHERE bca.finance_organization_id = $1 AND bca.id = $2`,
      [companyId, id],
    );
  ...
  async listAll(companyId: number): Promise<BankCashAccountRow[]> {
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE bca.finance_organization_id = $1 ORDER BY bca.code ASC`, [companyId]);
  ...
  async batchGet(companyId: number, codes: string[]): Promise<BankCashAccountRow[]> {
    ...
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE bca.finance_organization_id = $1 AND bca.code = ANY($2::text[]) ORDER BY bca.code ASC`, [companyId, codes]);
    return rows.map((row: Record<string, unknown>) => mapRow(row));
  }
  ...
  async filter(companyId: number, filters: Filter[], options?: ListOptions): Promise<BankCashAccountRow[]> {
    ...
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE bca.finance_organization_id = $1${sql} ${tail}`, queryParams);
    return rows.map((row: Record<string, unknown>) => mapRow(row));
  }
  ...
  async search(companyId: number, phrase: string, options?: ListOptions): Promise<BankCashAccountRow[]> {
    ...
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE bca.finance_organization_id = $1 AND (${likeParts.join(" OR ")}) ${tail}`, params);
    return rows.map((row: Record<string, unknown>) => mapRow(row));
  }
