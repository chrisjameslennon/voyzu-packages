import type { EntryType } from "@voyzu/finance/types/modules/core";
import type { DbExecutor } from "@voyzu/capability/db";
import type {
  TaxActivityReconciliationAuthorityOptionDto,
  TaxActivityReconciliationLineDto,
} from "@voyzu/finance/types/modules/company-reports";

import { TrialBalanceSnapshotRepo } from "../../../common/server/db/trial-balance-snapshot.repo";
import { resolveEffectiveSettingsCompanyId } from "../../../../common/server/settings-scope";

interface RawReconciliationLine {
  posting_date: string;
  document_type_code: string;
  document_code: string;
  document_id: string;
  source_document_id: string | null;
  tax_authority_code: string;
  tax_authority_name: string;
  tax_movement_type_code: string;
  entry_type: EntryType;
  base_currency_amount: number;
}

type LineKey = TaxActivityReconciliationLineDto["lineKey"];

const LINE_LABELS: Record<LineKey, string> = {
  OUTPUT_TAX_PAYABLE: "Output tax payable",
  INPUT_TAX_RECEIVABLE: "Less: Input tax recoverable",
  TAX_ADJUSTMENTS: "Tax adjustments",
  TAX_PAYMENTS: "Less: Tax payments",
  TAX_REFUNDS: "Plus: Tax refunds",
};

function lineKeyFor(row: RawReconciliationLine): LineKey {
  if (row.document_type_code === "TAX_PAYMENT") return "TAX_PAYMENTS";
  if (row.document_type_code === "TAX_REFUND") return "TAX_REFUNDS";
  if (row.document_type_code === "TAX_ADJUSTMENT") return "TAX_ADJUSTMENTS";
  if (row.tax_movement_type_code === "TAX_ON_PURCHASES") return "INPUT_TAX_RECEIVABLE";
  return "OUTPUT_TAX_PAYABLE";
}

function displayAmount(row: RawReconciliationLine): number {
  const key = lineKeyFor(row);
  if (key === "INPUT_TAX_RECEIVABLE") {
    return row.entry_type === "DEBIT" ? row.base_currency_amount : -row.base_currency_amount;
  }
  if (key === "TAX_PAYMENTS" || key === "TAX_REFUNDS") {
    return Math.abs(row.base_currency_amount);
  }
  return row.entry_type === "CREDIT" ? row.base_currency_amount : -row.base_currency_amount;
}

function contribution(line: TaxActivityReconciliationLineDto): number {
  if (line.lineKey === "INPUT_TAX_RECEIVABLE" || line.lineKey === "TAX_PAYMENTS") return -line.amount;
  return line.amount;
}

function dateString(value: unknown): string {
  if (value instanceof Date) {
    const y = value.getFullYear();
    const m = String(value.getMonth() + 1).padStart(2, "0");
    const d = String(value.getDate()).padStart(2, "0");
    return `${y}-${m}-${d}`;
  }
  return String(value);
}

function toLine(row: RawReconciliationLine): TaxActivityReconciliationLineDto {
  const lineKey = lineKeyFor(row);
  const isReturnLine = lineKey === "OUTPUT_TAX_PAYABLE" || lineKey === "INPUT_TAX_RECEIVABLE" || lineKey === "TAX_ADJUSTMENTS";
  return {
    sectionKey: isReturnLine ? "TAX_RETURN" : "SETTLEMENT",
    sectionLabel: isReturnLine ? "Tax return calculation" : "Settlement / reconciliation",
    lineKey,
    lineLabel: LINE_LABELS[lineKey],
    postingDate: row.posting_date,
    documentTypeCode: row.document_type_code,
    documentCode: row.document_code,
    documentRef: row.document_id,
    sourceDocumentRef: row.source_document_id,
    taxAuthorityCode: row.tax_authority_code,
    taxAuthorityName: row.tax_authority_name,
    amount: displayAmount(row),
  };
}

function rowFromRecord(row: Record<string, unknown>): RawReconciliationLine {
  return {
    posting_date: dateString(row.posting_date),
    document_type_code: String(row.document_type_code),
    document_code: String(row.document_code),
    document_id: String(row.document_id),
    source_document_id: row.source_document_id == null ? null : String(row.source_document_id),
    tax_authority_code: String(row.tax_authority_code),
    tax_authority_name: String(row.tax_authority_name),
    tax_movement_type_code: String(row.tax_movement_type_code),
    entry_type: row.entry_type === "CREDIT" ? "CREDIT" : "DEBIT",
    base_currency_amount: Number(row.base_currency_amount),
  };
}

export class TaxActivityReconciliationRepo {
  private readonly trialBalanceSnapshot: TrialBalanceSnapshotRepo;

  constructor(private readonly db: DbExecutor) {
    this.trialBalanceSnapshot = new TrialBalanceSnapshotRepo(db);
  }

  async listTaxAuthoritiesForCompany(companyId: number): Promise<TaxActivityReconciliationAuthorityOptionDto[]> {
    const { rows } = await this.db.query(
      `SELECT ta.code AS tax_authority_code, ta.name AS tax_authority_name
       FROM finance_organization fc
       JOIN organization c ON c.id = fc.organization_id
       JOIN tax_authority ta ON ta.country_code = c.country_code
       WHERE fc.id = $1
         AND ta.status = 'ACTIVE'
       ORDER BY ta.code ASC`,
      [companyId],
    );
    return rows.map((row: Record<string, unknown>) => ({
      taxAuthorityCode: String(row.tax_authority_code),
      taxAuthorityName: String(row.tax_authority_name),
    }));
  }

  async getAccrualLinesForPeriod(companyId: number, startDate: string, endDate: string): Promise<TaxActivityReconciliationLineDto[]> {
    const settingsCompanyId = await resolveEffectiveSettingsCompanyId(companyId, this.db);
    const { rows } = await this.db.query(
      `SELECT
         e.posting_date,
         h.document_type_code,
         h.code AS document_code,
         h.document_id,
         NULL::text AS source_document_id,
         ta.code AS tax_authority_code,
         ta.name AS tax_authority_name,
         l.tax_movement_type_code,
         CASE WHEN l.dr_cr = 'CR' THEN 'CREDIT' ELSE 'DEBIT' END AS entry_type,
         l.base_currency_amount::float AS base_currency_amount
       FROM tax_ledger_entry_header e
       JOIN tax_ledger_entry_line l ON l.tax_ledger_entry_header_id = e.id
       JOIN journal_header h ON h.id = e.journal_header_id
       JOIN tax_control_account tmt ON tmt.finance_organization_id = $4 AND tmt.code = l.tax_movement_type_code
       JOIN tax_authority ta ON ta.id = l.tax_authority_id
       WHERE e.finance_organization_id = $1
         AND e.status = 'POSTED'
         AND h.status = 'POSTED'
         AND tmt.status = 'ACTIVE'
         AND e.posting_date BETWEEN $2 AND $3
       ORDER BY e.posting_date ASC, h.code ASC, e.id ASC, l.line_number ASC`,
      [companyId, startDate, endDate, settingsCompanyId],
    );
    return rows.map((row: Record<string, unknown>) => toLine(rowFromRecord(row)));
  }

  total(lines: TaxActivityReconciliationLineDto[]): number {
    return lines.reduce((sum, line) => sum + contribution(line), 0);
  }

  async getTrialBalanceTaxMovement(companyId: number, startDate: string, endDate: string): Promise<number> {
    const settingsCompanyId = await resolveEffectiveSettingsCompanyId(companyId, this.db);
    const { rows } = await this.db.query(
      `SELECT gl_account_id::int AS gl_account_id
       FROM tax_control_account
       WHERE finance_organization_id = $1
         AND status = 'ACTIVE'`,
      [settingsCompanyId],
    );
    const glAccountIds = rows.map((row: Record<string, unknown>) => Number(row.gl_account_id));
    return this.trialBalanceSnapshot.getPeriodBalanceForGlAccounts(companyId, startDate, endDate, glAccountIds);
  }
}
