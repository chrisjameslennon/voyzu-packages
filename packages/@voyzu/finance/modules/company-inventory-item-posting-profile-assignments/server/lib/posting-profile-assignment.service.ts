...
export async function listPostingProfileAssignments(companyId: number): Promise<PostingAssignments> {
  ...
  const { items } = catalog ? await catalog.listItems({ organizationId: orgId }) : { items: [] };
  ...
  return { inventoryInstalled: catalog !== undefined, profiles, items: items.map((item) => { const postingProfileId = profileByItem.get(item.id) ?? null; return { ...item, postingProfileId, postingCode: postingProfileId == null ? null : codeByProfile.get(postingProfileId) ?? null }; }) };
}
