...
interface FinanceCompanyRow {
  ...
  finance_organization_id: number | null;
...
const SELECT_SQL = `
  SELECT
    ...
    fc.id::int AS finance_organization_id,
    ...
    EXISTS (SELECT 1 FROM journal_header j WHERE j.finance_organization_id = fc.id) AS has_postings
  FROM organization c
  ...
  LEFT JOIN finance_organization fc ON fc.organization_id = c.id AND fc.is_template = false
`;
...
export async function listSelectableFinanceCompaniesForCurrentUser(): Promise<OrganizationResponseDto[]> {
  ...
  const financeOrganizationIds = new Set(rows.map((row) => Number(row.organization_id)));
...
async function createFiscalCalendar(
  ...
  await db.query(
    `WITH proposed AS (
       SELECT fy.finance_organization_id, fy.id AS fiscal_year_id,
       ...
       WHERE fy.finance_organization_id = $1 AND fy.status = 'OPEN'
     )
     ...
     SELECT finance_organization_id, fiscal_year_id, upper(to_char(start_date, 'MON')),
  ...
  return withTransaction(async (db) => {
    ...
    if (!financeCompanyId) {
      const existing = await db.query(
        "SELECT id::int FROM finance_organization WHERE organization_id = $1 AND is_template = false",
        [company.id],
      );
...
async function copyTemplateSettings(financeCompanyId: number, db: DbExecutor): Promise<void> {
  const template = await db.query("SELECT id::int FROM finance_organization WHERE is_template = true ORDER BY id LIMIT 1");
  ...
  await db.query(
    `INSERT INTO gl_account_category (finance_organization_id, code, name, account_type, sequence, status)
     SELECT $1, code, name, account_type, sequence, status FROM gl_account_category
     WHERE finance_organization_id = $2 AND status = 'ACTIVE'
     ON CONFLICT (finance_organization_id, code) DO NOTHING`,
    [financeCompanyId, templateId],
  );
  await db.query(
    `INSERT INTO gl_account (finance_organization_id, code, name, account_type, account_category_id, status)
     ...
     LEFT JOIN gl_account_category source_category
       ON source_category.finance_organization_id = source.finance_organization_id AND source_category.id = source.account_category_id
     LEFT JOIN gl_account_category target_category
       ON target_category.finance_organization_id = $1 AND target_category.code = source_category.code
  ...
  for (const table of ["ar_control_account", "ap_control_account"] as const) {
    await db.query(
      `INSERT INTO ${table} (finance_organization_id, code, ledger, name, gl_account_id, status)
       SELECT $1, source.code, source.ledger, source.name, target_gl.id, source.status
       FROM ${table} source
       JOIN gl_account source_gl ON source_gl.finance_organization_id = source.finance_organization_id AND source_gl.id = source.gl_account_id
       JOIN gl_account target_gl ON target_gl.finance_organization_id = $1 AND target_gl.code = source_gl.code
       WHERE source.finance_organization_id = $2 AND source.status = 'ACTIVE'
       ON CONFLICT (finance_organization_id, code) DO NOTHING`,
      [financeCompanyId, templateId],
    );
  ...
  await db.query(
    `INSERT INTO tax_control_account (finance_organization_id, code, ledger, name, description, tax_family_code, gl_account_id, status)
  ...
  await db.query(
    `INSERT INTO inventory_control_account (finance_organization_id, code, ledger, name, description, gl_account_id, status)
  ...
  await db.query(
    `INSERT INTO bank_cash_control_account (
       finance_organization_id, code, ledger, type, gl_account_id, bank_name, bank_branch_name,
  ...
  await db.query(
    `INSERT INTO dimension (finance_organization_id, code, name, status)
  ...
  await db.query(
    `INSERT INTO dimension_value (finance_organization_id, dimension_id, name, status)
     SELECT $1, target_dimension.id, source_value.name, source_value.status
     FROM dimension_value source_value
     JOIN dimension source_dimension ON source_dimension.finance_organization_id = source_value.finance_organization_id AND source_dimension.id = source_value.dimension_id
     JOIN dimension target_dimension ON target_dimension.finance_organization_id = $1 AND target_dimension.code = source_dimension.code
     WHERE source_value.finance_organization_id = $2 AND source_value.status = 'ACTIVE'
  ...
  await db.query(
    `INSERT INTO financial_document_default (
     ...
     LEFT JOIN gl_account source_gl ON source_gl.finance_organization_id = source.finance_organization_id AND source_gl.id = source.gl_account_id
     LEFT JOIN gl_account target_gl ON target_gl.finance_organization_id = $1 AND target_gl.code = source_gl.code
     LEFT JOIN bank_cash_control_account source_bank ON source_bank.finance_organization_id = source.finance_organization_id AND source_bank.id = source.bank_cash_control_account_id
     LEFT JOIN bank_cash_control_account target_bank ON target_bank.finance_organization_id = $1 AND target_bank.code = source_bank.code
     WHERE source.finance_organization_id = $2 AND source.status = 'ACTIVE'
  ...
  await db.query(
    `INSERT INTO item_posting_profile (
     ...
     LEFT JOIN gl_account revenue_source ON revenue_source.finance_organization_id = source.finance_organization_id AND revenue_source.id = source.revenue_gl_account_id
     LEFT JOIN gl_account revenue_target ON revenue_target.finance_organization_id = $1 AND revenue_target.code = revenue_source.code
     LEFT JOIN gl_account cogs_source ON cogs_source.finance_organization_id = source.finance_organization_id AND cogs_source.id = source.cogs_gl_account_id
     LEFT JOIN gl_account cogs_target ON cogs_target.finance_organization_id = $1 AND cogs_target.code = cogs_source.code
     LEFT JOIN gl_account purchase_source ON purchase_source.finance_organization_id = source.finance_organization_id AND purchase_source.id = source.purchase_expense_gl_account_id
     LEFT JOIN gl_account purchase_target ON purchase_target.finance_organization_id = $1 AND purchase_target.code = purchase_source.code
     LEFT JOIN gl_account consumption_source ON consumption_source.finance_organization_id = source.finance_organization_id AND consumption_source.id = source.consumption_gl_account_id
     LEFT JOIN gl_account consumption_target ON consumption_target.finance_organization_id = $1 AND consumption_target.code = consumption_source.code
     LEFT JOIN gl_account gain_source ON gain_source.finance_organization_id = source.finance_organization_id AND gain_source.id = source.adjustment_gain_gl_account_id
     LEFT JOIN gl_account gain_target ON gain_target.finance_organization_id = $1 AND gain_target.code = gain_source.code
     LEFT JOIN gl_account loss_source ON loss_source.finance_organization_id = source.finance_organization_id AND loss_source.id = source.adjustment_loss_gl_account_id
     LEFT JOIN gl_account loss_target ON loss_target.finance_organization_id = $1 AND loss_target.code = loss_source.code
     WHERE source.finance_organization_id = $2 AND source.status = 'ACTIVE'
     ON CONFLICT (finance_organization_id, code) DO NOTHING`,
    [financeCompanyId, templateId],
  );
...
export async function updateFinanceCompany(
  ...
  return withTransaction(async (db) => {
    ...
    await db.query(
      `UPDATE finance_organization SET
...
export async function deleteFinanceCompanyForErpOrganization(
  organizationId: number,
  db: DbExecutor,
): Promise<void> {
  await db.query(
    `DELETE FROM finance_organization
