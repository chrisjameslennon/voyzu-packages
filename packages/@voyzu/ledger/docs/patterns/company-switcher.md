...
This is an internal development pattern for `@voyzu/ledger`. Finance owns selection data and behaviour; the platform `ContextSwitcher` owns the generic selector presentation and interaction. The UI labels this context “Organization” and prompts “Select Organization”.
...
```tsx
import { resolveServerCompanyHttpApiContext } from "@voyzu/ledger/common/server";
...
```text
/api/ledger/{companyCode}/inventory/items
```

Use the validated shared selection to construct browser URLs:

```ts
import { financeHttpApiUrl } from "@voyzu/ledger/common/client";
...
- Include `finance_organization_id` in reads, writes, joins, and existence checks.
