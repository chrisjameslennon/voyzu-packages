...
export async function handleList(request: NextRequest) {
  try { return ok(await listPostingProfileAssignments(await resolveHttpApiCompanyIdFromPath(request))); }
  catch (error) { return serverError(error); }
}

export async function handleAssign(request: NextRequest) {
  try { return ok(await assignPostingProfile(await resolveHttpApiCompanyIdFromPath(request), await parseBody<AssignPostingProfileRequest>(request))); }
  catch (error) { return serverError(error); }
}
