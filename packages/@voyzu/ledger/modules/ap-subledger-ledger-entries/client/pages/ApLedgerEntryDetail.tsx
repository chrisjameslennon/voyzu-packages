...
export function ApLedgerEntryDetail({
  ...
  from,
  fromCode,
  fallbackHref = "/ledger/subledgers/ap/ledger-entries",
  ...
  const viewItems: DropdownMenuItem[] = [
    { value: "counterparty", label: "View Counterparty", icon: "person", disabled: !counterpartyHref, onSelect: () => { if (counterpartyHref) router.push(counterpartyHref); } },
    {
      ...
      onSelect: () =>
        router.push(
          detailLinkWithBackContext(
            `/ledger/journals/${encodeURIComponent(entry.journalCode)}`,
            "apLedgerEntry",
            entry.code,
          ),
        ),
  ...
  const printablePath = `/ledger/subledgers/ap/ledger-entries/${encodeURIComponent(entry.code)}/document-printable`;
