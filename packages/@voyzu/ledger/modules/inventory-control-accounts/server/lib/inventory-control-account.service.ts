...
import type { InventoryControlAccountPatchRequestDto } from "../../types/index";
...
function toDto(row: InventoryControlAccountRow): InventoryControlAccountSettingResponseDto {
  ...
  return {
    ...
    linkedBy: [],
    audit: {
      created: { date: new Date(row.creation_date).toISOString(), actorType: row.creation_actor_type, userId: row.creation_user_id, mutationId: row.creation_mutation_id },
      updated: { date: new Date(row.updated_date).toISOString(), actorType: row.updated_actor_type, userId: row.updated_user_id, mutationId: row.updated_mutation_id },
    },
  };
}
