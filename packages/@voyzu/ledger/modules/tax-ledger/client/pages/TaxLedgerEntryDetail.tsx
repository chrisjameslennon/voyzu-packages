  ...
  const viewItems: DropdownMenuItem[] = [
    ...
    },
    {
      ...
      onSelect: () => {
        ...
        router.push(
          detailLinkWithBackContext(
            `/ledger/subledgers/ar/ledger-entries/${encodeURIComponent(entry.arSubledgerEntryCode)}`,
            "taxLedgerEntry",
            entry.code,
          ),
        );
    ...
    },
    {
      ...
      onSelect: () => {
        ...
        router.push(
          detailLinkWithBackContext(
            `/ledger/subledgers/ap/ledger-entries/${encodeURIComponent(entry.apSubledgerEntryCode)}`,
            "taxLedgerEntry",
            entry.code,
          ),
        );
    ...
    <div className={`${layout.detailView} ${layout.detailViewWithStatusRail}`}>
      <header className={layout.detailHeader}>
        ...
        <div className={layout.slotActions}>
          <div className={detailStyles.headerActions}><DetailBackButton fallbackHref={"/ledger/subledgers/tax/ledger-entries"} from={from} fromCode={fromCode} /></div>
        </div>
      </header>
      ...
      <aside className={layout.statusSection}>
        <div className={localStyles.statusRailStack}>
          ...
          <AuditPanel
            ...
            auditHref={`/settings/audit?entityType=tax_ledger_entry&entityId=${entry.id}`}
