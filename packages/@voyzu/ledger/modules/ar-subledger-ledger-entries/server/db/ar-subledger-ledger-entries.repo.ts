...
export interface ArDocumentApplicationRow {
  target_document_id: string;
  target_document_type_label: string;
  amount: number;
}

export interface ArDocumentAppliedTransactionRow {
  code: string;
  posting_date: string;
...
export class ArSubledgerRepo {
  ...
  async listDocumentLines(entryId: number): Promise<ArDocumentLineRow[]> {
    const { rows } = await this.db.query(
      `SELECT
       ...
       WHERE l.ar_subledger_entry_header_id = $1
         AND (
           l.line_type IN (
             'INVOICE_LINE', 'INVOICE_CANCELLATION_LINE', 'RECEIPT_ALLOCATION',
             'RECEIPT_UNAPPLIED', 'CREDIT_NOTE_LINE', 'OPENING_BALANCE_ITEM',
             'REFUND_APPLICATION', 'WRITE_OFF_APPLICATION'
           )
  ...
  async listDocumentApplications(entryId: number): Promise<ArDocumentApplicationRow[]> {
    const { rows } = await this.db.query(
      `SELECT
    ...
    return rows as unknown as ArDocumentApplicationRow[];
  }

  async listAppliedTransactions(entryId: number): Promise<ArDocumentAppliedTransactionRow[]> {
    const { rows } = await this.db.query(
    ...
    return rows as unknown as ArDocumentAppliedTransactionRow[];
  }
  ...
  async listEntries(companyId: number): Promise<ArSubledgerEntryRow[]> {
    ...
    const { rows } = await this.db.query(
      `SELECT ${AR_ENTRY_COLUMNS}
       ...
       JOIN ledger_ar_counterparty_snapshot c ON c.id = e.ar_counterparty_id
       JOIN LATERAL (
       ...
       JOIN ar_control_account ca ON ca.finance_organization_id = $2 AND ca.code = first_line.control_account_code
       JOIN gl_account ga ON ga.finance_organization_id = $2 AND ga.id = ca.gl_account_id
       ...
       LEFT JOIN LATERAL (
         ...
         FROM (
           ...
           FROM ar_subledger_entry_line line
           JOIN ar_control_account balance_ca ON balance_ca.finance_organization_id = $2 AND balance_ca.code = line.control_account_code
           JOIN gl_account balance_ga ON balance_ga.finance_organization_id = $2 AND balance_ga.id = balance_ca.gl_account_id
           WHERE line.ar_subledger_entry_header_id = e.id
  ...
  async getEntry(companyId: number, code: string): Promise<ArSubledgerEntryRow | null> {
    ...
    const { rows } = await this.db.query(
      `SELECT ${AR_ENTRY_COLUMNS}
       ...
       JOIN ar_control_account ca ON ca.finance_organization_id = $3 AND ca.code = first_line.control_account_code
       JOIN gl_account ga ON ga.finance_organization_id = $3 AND ga.id = ca.gl_account_id
       LEFT JOIN LATERAL (${HEADER_AMOUNT_SUBQUERY}) header_amount ON true
       LEFT JOIN LATERAL (
         SELECT json_agg(json_build_object(
           'controlAccountCode', grouped.control_account_code,
         ...
         FROM (
           SELECT line.control_account_code,
                  ...
                  SUM(CASE WHEN line.dr_cr = 'DR' THEN line.base_currency_amount ELSE -line.base_currency_amount END)::float AS balance
           FROM ar_subledger_entry_line line
           JOIN ar_control_account balance_ca ON balance_ca.finance_organization_id = $3 AND balance_ca.code = line.control_account_code
           JOIN gl_account balance_ga ON balance_ga.finance_organization_id = $3 AND balance_ga.id = balance_ca.gl_account_id
           WHERE line.ar_subledger_entry_header_id = e.id
           GROUP BY line.control_account_code, balance_ca.name, balance_ga.code, balance_ga.name
         ) grouped
       ) control_balances ON true
