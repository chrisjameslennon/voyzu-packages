...
function DocumentTab({
  ...
  const printablePath = `/ledger/subledgers/ar/ledger-entries/${encodeURIComponent(code)}/document-printable`;
...
export function ArLedgerEntryDocumentReport({
  ...
  from,
  fromCode,
  fallbackHref = "/ledger/subledgers/ar/ledger-entries",
  ...
  const viewItems: DropdownMenuItem[] = [
    { value: "counterparty", label: "View Counterparty", icon: "person", disabled: !counterpartyHref, onSelect: () => { if (counterpartyHref) router.push(counterpartyHref); } },
    {
      value: "journal",
      label: "View Journal",
      ...
      onSelect: () =>
        router.push(
          detailLinkWithBackContext(`/ledger/journals/${encodeURIComponent(entry.journalCode)}`, "arLedgerEntry", entry.code),
        ),
    },
    {
      value: "bank-cash",
      ...
      onSelect: () => {
        ...
        router.push(
          detailLinkWithBackContext(
            `/ledger/settings/bank-cash-accounts/${encodeURIComponent(entry.bankCashCode)}`,
            returnSource,
            entry.code,
          ),
        );
    ...
    },
    {
      ...
      onSelect: () => {
        ...
        router.push(
          detailLinkWithBackContext(
            `/ledger/subledgers/tax/ledger-entries/${encodeURIComponent(entry.taxLedgerEntryCode)}`,
            "arLedgerEntry",
            entry.code,
          ),
        );
      ...
      <aside className={detailLayout.statusSection}>
        <div className={localStyles.statusRailStack}>
          ...
          <AuditPanel
            ...
            auditHref={`/settings/audit?entityType=ar_subledger_entry&entityId=${entry.id}`}
