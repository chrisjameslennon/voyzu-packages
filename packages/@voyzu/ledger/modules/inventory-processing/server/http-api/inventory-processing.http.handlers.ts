...
export async function handleListRules(request: NextRequest): Promise<NextResponse<FinanceInventoryProcessingRule[] | ErrorResponse>> {
  try {
    return ok(await listFinanceInventoryProcessingRules(await resolveHttpApiCompanyIdFromPath(request)));
  } catch (error) {
...
export async function handleGetRule(
  ...
  try {
    ...
    const rule = await getFinanceInventoryProcessingRule(await resolveHttpApiCompanyIdFromPath(request), Number(id));
...
export async function handlePatchRule(
  ...
  try {
    ...
    return ok(await updateFinanceInventoryProcessingRule(
      await resolveHttpApiCompanyIdFromPath(request),
      Number(id),
      input,
    ));
  } catch (error) {
...
export async function handleListInventoryTransactions(
  ...
  try {
    return ok(await listFinanceInventoryActivities(await resolveHttpApiCompanyIdFromPath(request)));
  } catch (error) {
    return serverError(error);
  }
}
