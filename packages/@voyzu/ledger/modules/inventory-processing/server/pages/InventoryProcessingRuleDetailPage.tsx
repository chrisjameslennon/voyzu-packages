...
export async function InventoryProcessingRuleDetailPage({ context }: PageProps) {
  ...
  if (!rule) notFound();
  return <InventoryProcessingRuleDetail rule={rule} glAccounts={glAccounts.filter((account) => account.status === "ACTIVE" || account.id === rule.offsetGlAccountId)} httpApiPath={`/api/ledger/${encodeURIComponent(httpApiContext.companyCode)}/inventory-processing/rules/${rule.id}`} readOnly={settingsState.readOnly} />;
}
