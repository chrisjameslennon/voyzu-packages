...
export function InventoryProcessingRulesList({ rules }: { rules: FinanceInventoryProcessingRule[] }) {
  ...
  return <div className={layout.listView}>
    ...
    <div className={layout.listBody}><div className={layout.slotBody}><DataTable columns={columns} rows={visible} selectedIds={selectedIds} isAllSelected={allSelected} isSomeSelected={!allSelected && visible.some((rule) => selectedIds.has(rule.id))} onSelectAll={() => setSelectedIds((current) => allSelected ? new Set([...current].filter((id) => !visible.some((rule) => rule.id === id))) : new Set([...current, ...visible.map((rule) => rule.id)]))} onSelectOne={(id) => setSelectedIds((current) => { const next = new Set(current); next.has(id) ? next.delete(id) : next.add(id); return next; })} onRowClick={(row) => router.push(`/ledger/integration/inventory-processing/rules/${row.id}`)} currentPage={1} totalPages={1} onPageChange={() => undefined} totalCount={rules.length} filteredCount={visible.length} itemLabel="rules" hasData={rules.length > 0} emptyIcon="rule" emptyTitle="No inventory processing rules" emptyText="Inventory processing rules have not been configured" emptyFilterText="No rules match your search" mobileRender={(row) => <div className={listStyles.mobileCard}><div className={listStyles.mobileCode}>{label(row.inventoryDocumentType)} · {label(row.reasonCode)} · {label(row.direction)}</div><div className={listStyles.mobileName}><span className={listStyles.mobileNameText}>{label(row.action)}</span></div><div className={listStyles.mobileMeta}>{row.offsetGlAccount ? `${row.offsetGlAccount.code} ${row.offsetGlAccount.name}` : "No offset account"}</div></div>} /></div></div>
  </div>;
}
