...
export async function TaxControlAccountDetailPage({ context }: PageProps) {
  ...
  return <TaxControlAccountDetail account={account} glAccounts={glAccounts} httpApiPath={`/api/ledger/${encodeURIComponent(httpApiContext.companyCode)}/tax-control-accounts`} listPath="/ledger/settings/control-accounts/tax" auditPath="/settings/audit" readOnly={settingsState.readOnly} isArchived={settingsState.isArchived} />;
}
