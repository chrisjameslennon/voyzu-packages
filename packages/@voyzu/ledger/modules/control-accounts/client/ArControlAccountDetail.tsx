...
import type { ControlAccountResponseDto } from "../types/index";
...
export function ArControlAccountDetail({ account, glAccounts, httpApiPath, listPath = "/ledger/settings/control-accounts/ar", auditPath = "/settings/audit", readOnly = false, isArchived = false }: Props) {
  ...
  return (
    <div className={`${layoutStyles.detailView} ${layoutStyles.detailViewWithStatusRail}`}>
      ...
      <main className={layoutStyles.mainSection}><ArControlAccountDetailsForm account={currentAccount} glAccounts={selectableGlAccounts} glAccountId={glAccountId} disabled={readOnly || !canSelectAlternative} onGlAccountChange={setGlAccountId} /></main>
