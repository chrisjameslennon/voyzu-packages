  ...
  async listProtectedGlAccountLinks(companyId: number, codes: string[]): Promise<ProtectedGlAccountLinkRow[]> {
    ...
    const { rows } = await this.db.query(
      `SELECT ga.code AS gl_account_code,
       ...
       WHERE pc.finance_organization_id = $1 AND ga.code = ANY($2::text[])
       UNION ALL
       SELECT ga.code AS gl_account_code,
       ...
       WHERE ca.finance_organization_id = $1 AND ga.code = ANY($2::text[])
       UNION ALL
       SELECT ga.code AS gl_account_code,
              'CONTROL_ACCOUNT'::text AS source,
              ca.code AS source_code,
       ...
       UNION ALL
        ...
        WHERE tmt.finance_organization_id = $1 AND ga.code = ANY($2::text[])
        UNION ALL
        SELECT ga.code AS gl_account_code,
        ...
        WHERE bca.finance_organization_id = $1 AND ga.code = ANY($2::text[])`,
      [companyId, codes],
    );
