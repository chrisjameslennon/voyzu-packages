...
export class ArReceiptPostingRepo {
  ...
  getCounterparty(companyId: number, code: string) { return this.one(`SELECT cp.*, c.currency_code AS country_currency_code FROM ledger_ar_counterparty_snapshot cp JOIN country c ON c.code = cp.country_code WHERE cp.finance_organization_id = $1 AND cp.code = $2`, [companyId, code], counterpartyRow); }
  ...
  async upsertCounterparty(companyId: number, input: { code?: string | null; name: string; status: "ACTIVE" | "INACTIVE"; country_code: string; state_or_province_code?: string | null }): Promise<CounterpartyRow & { was_created: boolean }> {
    const { rows } = await this.db.query(`WITH upserted AS (
      INSERT INTO ledger_ar_counterparty_snapshot (finance_organization_id, code, name, status, country_code, tax_region_or_province, creation_date, creation_actor_type, updated_date, updated_actor_type)
