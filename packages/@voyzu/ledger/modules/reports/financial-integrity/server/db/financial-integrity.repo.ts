...
import type { AccountType, DrCr } from "../../../../common/types/index";
import type { FinancialIntegrityDocumentTypeDto, FinancialIntegrityInventoryLedgerLineDto, FinancialIntegrityLedgerLineDto } from "../../types/financial-integrity.response.dto";
  ...
  async getLedgerLines(companyId: number, fromDate: string, toDate: string): Promise<FinancialIntegrityLedgerLineDto[]> {
    const { rows } = await this.db.query(
      `WITH source_lines AS (
         ...
         JOIN gl_account ga ON ga.finance_organization_id = jh.finance_organization_id AND ga.id = jl.gl_account_id
    ...
    const { rows } = await this.db.query(
      `SELECT
         ...
         (SELECT COUNT(*)::int
          FROM journal_line jl
          JOIN journal_header jh ON jh.id = jl.journal_header_id
          LEFT JOIN gl_account ga ON ga.finance_organization_id = jh.finance_organization_id AND ga.id = jl.gl_account_id
          WHERE jh.finance_organization_id = $1
            AND jh.posting_date >= $2
            AND jh.posting_date <= $3
