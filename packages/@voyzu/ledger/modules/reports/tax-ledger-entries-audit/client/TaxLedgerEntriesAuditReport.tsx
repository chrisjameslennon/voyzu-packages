...
export function TaxLedgerEntriesAuditReport({
  ...
  const fetchData = useCallback(async (companyId: number, rangeStart: string, rangeEnd: string) => {
    ...
    try {
      ...
      const res = await fetch(await financeHttpApiUrl(`/reports/tax-ledger-entries-audit?${params.toString()}`));
      if (!res.ok) return;
  ...
  const printableBase = "/ledger/reports/tax-ledger-entries-audit/printable";
