...
const COLUMNS: readonly string[] = [
  "finance_organization_id", "document_code", "code", "name", "target_type", "allowed_account_types", "override_property_name", "override_scope",
...
const JOIN_SQL = `
  LEFT JOIN gl_account ga ON ga.finance_organization_id = p.finance_organization_id AND ga.id = p.gl_account_id
  LEFT JOIN bank_cash_control_account bca ON bca.finance_organization_id = p.finance_organization_id AND bca.id = p.bank_cash_control_account_id
  LEFT JOIN gl_account bank_ga ON bank_ga.finance_organization_id = p.finance_organization_id AND bank_ga.id = bca.gl_account_id`;
...
export class FinancialDocumentDefaultRepo {
  ...
  async insert(row: InsertFinancialDocumentDefaultRow): Promise<FinancialDocumentDefaultRow> {
    ...
    return this.getByKeyInternal(row.finance_organization_id, String(rows[0].document_code), String(rows[0].code));
  }

  async get(companyId: number, documentCode: string, code: string): Promise<FinancialDocumentDefaultRow | null> {
    const { rows } = await this.db.query(
      `${SELECT_SQL} FROM ${TABLE} p ${JOIN_SQL} WHERE p.finance_organization_id = $1 AND p.document_code = $2 AND p.code = $3`,
  ...
  async update(companyId: number, documentCode: string, code: string, row: UpdateFinancialDocumentDefaultRow): Promise<FinancialDocumentDefaultRow> {
    ...
    const sql = `UPDATE ${TABLE} SET ${sets.join(", ")} WHERE finance_organization_id = $${vals.length - 2} AND document_code = $${vals.length - 1} AND code = $${vals.length} RETURNING document_code, code`;
  ...
  async delete(companyId: number, documentCode: string, code: string): Promise<void> {
    await this.db.query(`DELETE FROM ${TABLE} WHERE finance_organization_id = $1 AND document_code = $2 AND code = $3`, [companyId, documentCode, code]);
  }

  async listAll(companyId: number): Promise<FinancialDocumentDefaultRow[]> {
    const { rows } = await this.db.query(
      `${SELECT_SQL} FROM ${TABLE} p ${JOIN_SQL} WHERE p.finance_organization_id = $1 ORDER BY p.document_code ASC, p.code ASC`,
      [companyId],
    );
  ...
  async filter(companyId: number, filters: Filter[], options?: ListOptions): Promise<FinancialDocumentDefaultRow[]> {
    ...
    const scopedWhere = shiftedWhere ? `WHERE p.finance_organization_id = $1 AND ${shiftedWhere.slice("WHERE ".length)}` : "WHERE p.finance_organization_id = $1";
  ...
  async search(companyId: number, phrase: string, options?: ListOptions): Promise<FinancialDocumentDefaultRow[]> {
    ...
    const whereSql = `WHERE p.finance_organization_id = $1 AND (${likeParts.join(" OR ")}
    ...
    const { rows } = await this.db.query(
      `${SELECT_SQL} FROM ${TABLE} p ${JOIN_SQL}
       WHERE (p.document_code, p.code) IN (
         ...
         AND p.finance_organization_id = $1
  ...
  async getGlAccount(companyId: number, glAccountId: number): Promise<{ id: number; accountType: "ASSET" | "LIABILITY" | "EQUITY" | "REVENUE" | "EXPENSE"; status: "ACTIVE" | "INACTIVE" } | null> {
    const { rows } = await this.db.query(
      `SELECT id, account_type, status FROM gl_account WHERE finance_organization_id = $1 AND id = $2`,
