...
import type { OrganizationSelectionResponseDto } from "../../organization-finance/types/organization-selection.dto";
...
async function selectedFinanceBase(): Promise<string> {
  if (!selectedFinanceBasePromise) {
    selectedFinanceBasePromise = fetch("/api/ledger/company-selection", { cache: "no-store" })
      .then(async (response) => {
        ...
        return `/api/ledger/${encodeURIComponent(company.code)}`;
      });
  }
