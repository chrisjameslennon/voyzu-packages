DROP TRIGGER IF EXISTS ar_counterparty_audit_trigger ON ledger_ar_counterparty_snapshot;
CREATE TRIGGER ar_counterparty_audit_trigger
  BEFORE INSERT OR UPDATE OR DELETE ON ledger_ar_counterparty_snapshot
  FOR EACH ROW
  EXECUTE FUNCTION audit_trigger_fn('@voyzu/ledger');

DROP TRIGGER IF EXISTS ap_counterparty_audit_trigger ON ledger_ap_counterparty_snapshot;
CREATE TRIGGER ap_counterparty_audit_trigger
  BEFORE INSERT OR UPDATE OR DELETE ON ledger_ap_counterparty_snapshot
  FOR EACH ROW
  EXECUTE FUNCTION audit_trigger_fn('@voyzu/ledger');
