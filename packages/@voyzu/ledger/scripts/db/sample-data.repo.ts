...
export class FinanceSampleDataRepo {
  ...
  deleteArCounterparties(companyIds: number[]) {
    return this.db.query(`DELETE FROM ledger_ar_counterparty_snapshot WHERE finance_organization_id = ANY($1)`, [companyIds]);
  }

  deleteApCounterparties(companyIds: number[]) {
    return this.db.query(`DELETE FROM ledger_ap_counterparty_snapshot WHERE finance_organization_id = ANY($1)`, [companyIds]);
  }
