...
import styles from "./template-audit-list.module.css";

const PACKAGE_CODE = "@voyzu/template";
const PAGE_SIZE = 50;
...
export function TemplateAuditEventList({ initialFilters = {} }: { initialFilters?: Record<string, string> }) {
  ...
  const [mutationId, setMutationId] = useState(initialFilters.mutationId ?? "");
  ...
  const backCode = initialFilters.from === "template" ? initialFilters.fromCode : undefined;
  ...
  }, [cursor, dateFrom, dateTo, entityCode, entityId, entityType, mutationId, search, userCode]);
  ...
  const detailQuery = useMemo(() => {
    ...
    if (dateTo) params.set("dateTo", dateTo);
    if (mutationId) params.set("mutationId", mutationId);
    for (const key of ["from", "fromCode"] as const) {
      if (initialFilters[key]) params.set(key, initialFilters[key]);
    }
    return params.toString();
  }, [dateFrom, dateTo, entityCode, entityId, entityType, initialFilters, mutationId, search, userCode]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        if (!fetchResponse.ok) {
          const body = await fetchResponse.json().catch(() => null) as { message?: string } | null;
          setErrorMessage(body?.message ?? "The audit events could not be loaded");
          return;
        }
        ...
        if (!(error instanceof DOMException && error.name === "AbortError")) {
          setErrorMessage("The audit events could not be loaded");
        }
      } finally {
  ...
  const downloadCsv = async (rows: AuditEventResponseDto[], filename: string) => {
    ...
    if (!response.ok) {
      setErrorMessage("The audit export could not be created");
      return;
    }
  ...
  const exportAll = async () => {
    ...
    if (!response.ok) {
      setErrorMessage("The audit export could not be loaded");
      return;
    }
    await downloadCsv(await response.json() as AuditEventResponseDto[], "template_audit_full_dataset");
  };
  ...
  const exportItems = useMemo<DropdownMenuItem[]>(() => [
    {
      ...
      onSelect: () => void downloadCsv(items.filter((event) => selectedIds.has(event.id)), "template_audit_selected"),
    },
    {
      value: "current-page",
      label: `Current page (${items.length})`,
      icon: "visibility",
      onSelect: () => void downloadCsv(items, "template_audit_current_page"),
    },
    {
      value: "full-dataset",
  ...
  const activeFilters = entityType || entityCode || entityId || userCode || mutationId || dateFrom || dateTo || search;
    ...
    <div className={styles.page}>
      ...
      <div className={styles.titleRow}>
        <div className={layout.slotTitle}>
          ...
          <h1 className={`${typography.pageTitle} ${layout.pageTitleResponsive}`}>Template Audit Log</h1>
          <div className={layout.slotTitleByline}><p className={typography.headingByline}>Changes recorded by the Templates package.</p></div>
        </div>
        {backCode ? <div className={styles.titleActions}><DetailBackButton fallbackHref={`/template/${encodeURIComponent(backCode)}`} /></div> : null}
      </div>
      <ValidationAlert errors={errorMessage ? [errorMessage] : []} visible={!!errorMessage} onDismiss={() => setErrorMessage("")} />
      ...
      <div className={styles.filterSearchRow}>
        <div className={styles.filterRow}>
          ...
          <div className={styles.filterField}><label className={styles.filterLabel}>Entity Code</label><Input containerClassName={styles.filterInput} placeholder="e.g. EXAMPLE_ONE" value={entityCode} onChange={(event) => { setEntityCode(event.target.value); resetPage(); }} /></div>
      ...
      <div className={styles.actionsRow}>
        ...
        <div className={styles.actionsRight}>
          <Button variant="plain" icon="sync" className={refreshing ? styles.spinning : undefined} disabled={refreshing} title="Refresh" onClick={() => { setRefreshing(true); setRefreshKey((value) => value + 1); }} />
      ...
      <DataTable
        ...
        onRowClick={(event) => router.push(`/template/audit/${event.id}${detailQuery ? `?${detailQuery}` : ""}`)}
        currentPage={page}
        ...
        emptyText="Template changes will appear here."
