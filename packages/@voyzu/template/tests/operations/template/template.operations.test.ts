...
import {
  ...
  searchTemplates,
  updateTemplate,
} from "../../../modules/template/operations";
...
function input(overrides: Partial<TemplateCreateRequestDto> = {}): TemplateCreateRequestDto {
  ...
  return { code, description: `Operation test ${code}`, ...overrides };
}
...
test("operations reject duplicate codes and redundant transitions", async () => {
  const request = input();
  const created = await createTemplate(request);
