...
export async function ItemDetailPage({ sku }: { sku?: string }) {
  ...
  const [item, categories, items, postingProfiles] = await Promise.all([
    getItem(organization.id, decodeURIComponent(sku)), listItemCategories(organization.id), listItems(organization.id), listPostingProfileOptions(),
  ]);
  if (!item) notFound();
  return <ItemDetail item={item} categories={categories} itemOptions={items} postingProfiles={postingProfiles} />;
}
