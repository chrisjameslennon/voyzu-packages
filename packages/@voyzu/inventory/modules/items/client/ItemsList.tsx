...
import type { ItemCategoryOptionDto, ItemCreateRequestDto, ItemResponseDto } from "../types/item.types";
...
export function ItemsList({ items, categories, nextSku }: { items: ItemListRow[]; categories: ItemCategoryOptionDto[]; nextSku: string; }) {
  ...
  const resetCreate = () => { setSku(""); setAutoSku(false); setName(""); setCategoryId(""); setUnit("each"); setQuantityTracked(true); setServerError(""); validation.reset(); };
  ...
  const create = async () => { setServerError(""); if (!validation.attempt()) return; setSaving(true); try { const payload: ItemCreateRequestDto = { ...(!autoSku && { sku: sku.trim().toUpperCase() }), name: name.trim(), categoryId: Number(categoryId), unit: quantityTracked && unit ? unit : null, quantityTracked };
