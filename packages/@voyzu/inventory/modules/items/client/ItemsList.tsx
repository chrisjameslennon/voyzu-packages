...
const columns: DataTableColumn<ItemListRow>[] = [
  ...
  { key: "itemType", label: "Type", width: "8rem", render: (row) => row.itemType === "ASSEMBLY" ? "Assembly" : "Item" },
...
export function ItemsList({ items, categories }: { items: ItemListRow[]; categories: ItemCategoryOptionDto[]; }) {
  ...
  const create = async () => { setServerError(""); if (!validation.attempt()) return; if (autoSku && reservedId === null) { setServerError("Generate an automatic SKU before creating the item"); return; } setSaving(true); try { const payload: ItemCreateRequestDto = { ...(autoSku ? { reservedId: reservedId! } : { sku: sku.trim().toUpperCase() }), name: name.trim(), categoryId: Number(categoryId), unit: quantityTracked && unit ? unit : null, quantityTracked };
    ...
    const item = await response.json() as ItemResponseDto; setRows((current) => [...current, { id: item.id, sku: item.sku, name: item.name, category: item.category?.name ?? null, itemType: item.itemType, unit: item.unit, quantityTracked: item.quantityTracked, status: item.status }].sort((a, b) => a.sku.localeCompare(b.sku)));
