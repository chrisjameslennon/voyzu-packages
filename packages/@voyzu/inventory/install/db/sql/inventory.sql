...
SET source_business_object='STOCK_COUNT', source_id=count.id
...
FROM stock_count count
WHERE transaction.organization_id=count.organization_id
  AND transaction.source_business_object='STOCK_COUNT'
...
WHERE transaction_type='ADJUSTMENT'
  AND source_business_object='STOCK_COUNT'
  AND code LIKE 'INV-STCOUNT-%';

UPDATE inventory_transaction
SET source_business_object=NULL
...
ALTER TABLE inventory_transaction
  ADD CONSTRAINT ck_inventory_transaction_source_link CHECK (
    (source_business_object IS NULL AND source_id IS NULL)
    OR (source_business_object IS NOT NULL AND source_id IS NOT NULL)
  );
