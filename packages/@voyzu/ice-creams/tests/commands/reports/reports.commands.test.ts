...
import { createIceCream, listIceCreamFlavors } from "../../../modules/ice-creams/commands";
import { getAllIceCreamsReport } from "../../../modules/reports/commands";
...
before(async () => {
  ...
  assert.ok(flavor, "An active ice-cream flavour is required for report command tests");
  await createIceCream({
    code,
    name: "Report command test",
...
test("getAllIceCreamsReport exposes report rows through the public command", async () => {
  const row = (await getAllIceCreamsReport()).find((candidate) => candidate.code === code);
  assert.ok(row);
  assert.equal(row.name, "Report command test");
