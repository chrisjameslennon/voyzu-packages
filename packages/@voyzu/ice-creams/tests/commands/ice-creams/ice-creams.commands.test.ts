...
import {
  ...
  searchIceCreams,
  updateIceCream,
} from "../../../modules/ice-creams/commands";
...
function input(overrides: Partial<IceCreamCreateRequestDto> = {}): IceCreamCreateRequestDto {
  ...
  return {
    code,
    name: `Command test ${code}`,
    flavorCode,
    supplier: "Command Test Creamery",
    ...overrides,
  };
}
...
before(async () => {
  ...
  assert.ok(flavor, "An active ice-cream flavour is required for command tests");
...
test("commands reject duplicate codes, unknown flavors, and redundant transitions", async () => {
  const request = input();
  const created = await createIceCream(request);
