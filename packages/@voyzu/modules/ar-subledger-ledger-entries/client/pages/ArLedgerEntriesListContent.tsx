...
import { CompanyPageTitleBadges, getDrCrColor, getStatusSemanticColor } from "@voyzu/modules/common/client";
import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/navigation";

import type { ArSubledgerEntryResponseDto } from "@voyzu/types/modules/ar-subledger";
...
import localStyles from "./ar-subledger-list.module.css";
...
export function ArLedgerEntriesListContent({ entries }: { entries: ArSubledgerEntryResponseDto[] }) {
  ...
  const balances = useMemo(() => {
    const map = new Map<string, NonNullable<ArSubledgerEntryResponseDto["controlAccountBalances"]>[number]>();
  ...
  const exportItems = useMemo<DropdownMenuItem[]>(() => [
    { value: "selected", label: `Selected (${selectedRows.length})`, icon: "check_box", disabled: selectedRows.length === 0, onSelect: () => { void handleExport(selectedRows, "ar_ledger_entries_selected"); } },
    { value: "current-view", label: `Current view (${filtered.length})`, icon: "visibility", disabled: filtered.length === 0, onSelect: () => { void handleExport(filtered, "ar_ledger_entries_current_view"); } },
    { value: "full-dataset", label: `Full dataset (${rows.length})`, icon: "database", disabled: rows.length === 0, onSelect: () => { void handleExport(rows, "ar_ledger_entries_full_dataset"); } },
  ], [filtered, rows, selectedRows]);
  return (
    <div className={`${layout.listView} vz-grid-12`}>
      <header className={layout.listHeader}><div className={layout.slotBreadcrumb}><Breadcrumbs /></div><div className={`${layout.slotTitle} ${localStyles.titleSlot}`}><div className={listStyles.titleIcon}><span className={`material-symbols-outlined ${listStyles.titleIconSymbol}`}>receipt_long</span></div><h1 className={`${typography.pageTitle} ${layout.pageTitleResponsive}`}>AR Ledger Entries</h1><div className={layout.slotTitleMeta}><CompanyPageTitleBadges /></div><div className={layout.slotTitleByline}><p className={typography.headingByline}>Accounts receivable subledger entries recorded from customer documents and receipts.</p></div></div><div className={`${layout.slotSearch} ${localStyles.balancesSlot}`}><section className={localStyles.balancesCard} aria-label="Control account balances"><h2 className={localStyles.balancesTitle}>Control Account Balances</h2><div className={localStyles.balanceRows}>{balances.map((balance) => <div key={balance.controlAccountCode} className={localStyles.balanceRow}><span className={localStyles.balanceLabel}>{balance.glAccountName}</span><span className={localStyles.codeChip}>{balance.glAccountCode}</span><span className={localStyles.balanceValue}>{balanceMoney(balance.balance)}</span></div>)}<div className={`${localStyles.totalRow} ${localStyles.balanceTotalRow}`}><span className={localStyles.balanceLabel}>Total</span><span className={localStyles.balanceValue}>{balanceMoney(totalBalance)}</span></div></div></section></div></header>
      <div className={layout.listToolbar}><div className={layout.slotToolbarLeft}><FilterPanel tabs={filterTabs} filters={activeFilters} onApply={(filters) => { setActiveFilters(filters); setCurrentPage(1); }} onClear={() => { setActiveFilters({}); setCurrentPage(1); }} onRemoveFilter={removeFilter} showChips={false} /></div><div className={layout.slotToolbarSearch}><Input search containerClassName={layout.slotSearchControl} placeholder="Search AR entries..." value={search} onChange={(event) => { setSearch(event.target.value); setCurrentPage(1); }} /></div><div className={layout.slotToolbarRight}><div className={listStyles.toolbarActions}><Button variant="plain" icon="sync" className={refreshing ? listStyles.spinning : undefined} disabled={refreshing} title="Refresh" onClick={refresh} /><DropdownMenu trigger={<Button variant="plain" icon="file_download" title="Export" />} items={exportItems} alignment="right" width={260} /></div></div></div>
      ...
      <div className={layout.listBody}><div className={layout.slotBody}><DataTable columns={columns} rows={paginated} selectedIds={selectedIds} isAllSelected={isAllSelected} isSomeSelected={isSomeSelected} onSelectAll={() => setSelectedIds((current) => isAllSelected ? new Set([...current].filter((id) => !paginated.some((row) => row.id === id))) : new Set([...current, ...paginated.map((row) => row.id)]))} onSelectOne={(id) => setSelectedIds((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; })} onRowClick={(row) => router.push(`/finance/subledgers/ar/ledger-entries/${encodeURIComponent(row.code)}`)} currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalCount={rows.length} filteredCount={filtered.length} itemLabel="entries" hasData={rows.length > 0} emptyIcon="receipt_long" emptyTitle="No AR ledger entries found" emptyText="No AR ledger entries have been posted" emptyFilterText="No entries match your search" mobileRender={(row) => <div className={listStyles.mobileCard}><div className={listStyles.mobileCode}>{row.code}</div><div className={listStyles.mobileName}><span className={listStyles.mobileNameText}>{row.counterpartyName}</span></div><div className={listStyles.mobileMeta}>{row.documentId} - {money(row.baseCurrencyAmount)}</div><Badge variant="soft" size="x-small" color={getStatusSemanticColor(row.status)}>{row.status}</Badge></div>} /></div></div>
    </div>
  );
}
