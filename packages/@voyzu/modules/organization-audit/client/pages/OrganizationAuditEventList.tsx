...
import { financeApiUrl, getAuditActionColor } from "@voyzu/modules/common/client";
import type { AuditEventListResponseDto } from "@voyzu/types/modules/audit";
import type { AuditEventResponseDto } from "@voyzu/types/modules/audit";
import type { OrganizationAuditCountResponseDto } from "@voyzu/types/modules/audit";
...
import styles from "./organization-audit-event-list.module.css";
...
const API_BASE_PATH = "/api/organization/audit";
const ROUTE_BASE_PATH = "/organization/audit";
...
const columns: DataTableColumn<AuditEventResponseDto>[] = [
  ...
  { key: "entityCode", label: "Entity Code", width: 180, render: (e) => e.entityCode ?? "-" },
...
export function OrganizationAuditEventList({
  companies,
  ...
  const fetchYears = useCallback(async (cId: number) => {
    if (!cId) {
      setFinancialYears([]);
      setSelectedYearCode("");
      return;
    ...
    const years = (await res.json()) as FinancialYearResponseDto[];
    ...
    if (nextYear) {
      if (rangePreset === "all-dates") {
        setDateFrom("");
        setDateTo("");
        return;
      }
      const range = rangeForPreset(rangePreset, nextYear);
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_PATH}/count`, { signal: controller.signal })
      .then((r) => r.json())
      .then((response: OrganizationAuditCountResponseDto) => setTotalDbCount(response.count))
      .catch(() => { /* ignore abort */ });
  ...
  const activeChips = useMemo((): ChipDef[] => {
    ...
    return chips;
  }, [companyId, selectedCompany, entityType, entityCode, entityId, mutationId, actorId, search, dateFrom, dateTo, dateFilterSuppressed, clearDateRange]);
      ...
      <div className={styles.titleRow}>
        <div className={layout.slotTitle}>
          ...
          <div className={layout.slotTitleByline}>
            <p className={typography.headingByline}>The organization audit log shows a complete record of every change made to data within the organization, including data changes made to any company.</p>
          </div>
        </div>
        <div className={styles.titleActions}>
          {backFromCode ? (
            <DetailBackButton fallbackHref="/organization" from="organizationAudit" fromCode={backFromCode} />
          ) : null}
        </div>
      </div>
