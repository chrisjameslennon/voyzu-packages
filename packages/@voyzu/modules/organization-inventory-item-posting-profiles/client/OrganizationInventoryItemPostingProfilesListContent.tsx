...
import type { ItemPostingProfileCreateRequestDto, ItemPostingProfileGlRefDto, ItemPostingProfileResponseDto } from "@voyzu/types/modules/inventory-item-posting-profiles";
...
export function OrganizationInventoryItemPostingProfilesListContent({
  ...
  const canDeactivateSelection = hasSelection && selectedRows.some((row) => row.status !== "INACTIVE") && selectedRows.every((row) => Deactivate({ code: row.profile_code, linkedBy: row.linkedBy }).length === 0);
  const canDeleteSelection = hasSelection && selectedRows.every((row) => Delete({ code: row.profile_code, linkedBy: row.linkedBy }).length === 0);
