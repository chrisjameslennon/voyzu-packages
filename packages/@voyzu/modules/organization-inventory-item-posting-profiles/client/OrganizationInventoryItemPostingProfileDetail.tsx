...
import { AssignGLAccount, Deactivate, Delete, PostingAccountEnabled, PostingAccountRequired } from "@voyzu/modules/common/inventory-item-posting-profiles/domain/operation-policy";
...
import type { ItemPostingProfilePatchRequestDto, ItemPostingProfileResponseDto } from "@voyzu/types/modules/inventory-item-posting-profiles";
    ...
    <div className={`${layout.detailView} ${layout.detailViewWithStatusRail}`}>
      <header className={layout.detailHeader}>
        ...
        <div className={layout.slotActions}>
          <div className={detailStyles.headerActions}>
            ...
            <Button variant="secondary" icon="check_circle" disabled={profile.status === "ACTIVE"} onClick={() => { void transitionStatus("activate"); }}>Activate</Button>
            <Button variant="secondary" icon="block" disabled={profile.status === "INACTIVE" || Deactivate({ code: profile.profile_code, linkedBy: profile.linkedBy }).length > 0} onClick={() => { void transitionStatus("deactivate"); }}>Deactivate</Button>
            <Button variant="danger" icon="delete" disabled={Delete({ code: profile.profile_code, linkedBy: profile.linkedBy }).length > 0} onClick={() => { void deleteProfile(); }} />
          </div>
        </div>
      ...
      <aside className={layout.statusSection}>
        ...
        <AuditPanel
          ...
          auditHref={`/organization/audit?entityType=item_posting_profile&entityCode=${encodeURIComponent(profile.profile_code)}`}
