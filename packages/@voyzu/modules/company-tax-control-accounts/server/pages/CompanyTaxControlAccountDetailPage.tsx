...
export async function CompanyTaxControlAccountDetailPage({ code }: { code?: string }) {
  ...
  return <CompanyTaxControlAccountDetail account={account} glAccounts={glAccounts} apiPath={`/api/finance/${encodeURIComponent(apiContext.companyCode)}/tax-control-accounts`} listPath="/finance/settings/control-accounts/tax" auditPath="/finance/audit" readOnly={settingsState.readOnly} usesOrganizationStandardSettings={settingsState.usesOrganizationStandardSettings} isArchived={settingsState.isArchived} />;
}
