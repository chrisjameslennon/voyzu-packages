...
export class InventoryItemRepo {
  ...
  async resolveCategoryId(companyId: number, categoryCode: string): Promise<number> {
    const category = await this.db.query(`SELECT id::int FROM inventory_category WHERE company_id = $1 AND code = $2`, [companyId, categoryCode]);
  ...
  async insert(row: InsertInventoryItemRow): Promise<InventoryItemRow> {
    const categoryId = await this.resolveCategoryId(row.company_id, row.category_code);
    await this.db.query(
      ...
      [
        row.company_id, row.code, row.name, row.description, row.item_type, categoryId, row.unit_code,
  ...
  async listAll(companyId: number): Promise<InventoryItemRow[]> {
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE ii.company_id = $1 ORDER BY ii.code`, [companyId]);
    return rows.map((row: Record<string, unknown>) => mapRow(row));
  }

  async get(companyId: number, code: string): Promise<InventoryItemRow | null> {
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE ii.company_id = $1 AND ii.code = $2`, [companyId, code]);
    return rows[0] ? mapRow(rows[0]) : null;
  }

  async batchGet(companyId: number, codes: string[]): Promise<InventoryItemRow[]> {
    if (!codes.length) return [];
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE ii.company_id = $1 AND ii.code = ANY($2::text[]) ORDER BY ii.code`, [companyId, codes]);
    return rows.map((row: Record<string, unknown>) => mapRow(row));
  }
  ...
  async filter(companyId: number, filters: Filter[], options?: ListOptions): Promise<InventoryItemRow[]> {
    ...
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE ii.company_id = $1${sql} ${tail}`, queryParams);
    return rows.map((row: Record<string, unknown>) => mapRow(row));
  }
  ...
  async search(companyId: number, phrase: string, options?: ListOptions): Promise<InventoryItemRow[]> {
    ...
    const { rows } = await this.db.query(`${SELECT_SQL} WHERE ii.company_id = $1 AND (${likeParts.join(" OR ")}) ${tail}`, params);
    return rows.map((row: Record<string, unknown>) => mapRow(row));
  }
  ...
  async patch(companyId: number, code: string, updates: PatchInventoryItemRow): Promise<InventoryItemRow> {
    ...
    if (updates.category_code) {
      const { rows } = await this.db.query(`SELECT id::int FROM inventory_category WHERE company_id = $1 AND code = $2`, [companyId, updates.category_code]);
  ...
  async delete(companyId: number, code: string): Promise<void> {
    const { rowCount } = await this.db.query(`DELETE FROM inventory_item WHERE company_id = $1 AND code = $2`, [companyId, code]);
