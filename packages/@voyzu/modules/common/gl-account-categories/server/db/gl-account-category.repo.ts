...
const COMPANIES_WITH_POSTINGS_SQL = `COALESCE(ARRAY(
           ...
           JOIN gl_account ga ON ga.company_id = gac.company_id
           ...
           WHERE source_company.id = gac.company_id
...
const SELECT_WITH_DERIVED = `
  SELECT gac.*,
         ...
         COALESCE((
           ...
           WHERE linked_account.company_id = gac.company_id
...
export class GlAccountCategoryRepo {
  ...
  async get(companyId: number, code: string): Promise<GlAccountCategoryRow | null> {
    const { rows } = await this.db.query(
      `${SELECT_WITH_DERIVED} WHERE gac.company_id = $1 AND gac.code = $2 AND gac.status != 'DELETED'`,
      [companyId, code],
    );
  ...
  async getById(companyId: number, id: number): Promise<GlAccountCategoryRow> {
    const { rows } = await this.db.query(
      `${SELECT_WITH_DERIVED} WHERE gac.company_id = $1 AND gac.id = $2`,
      [companyId, id],
    );
  ...
  async listAll(companyId: number): Promise<GlAccountCategoryRow[]> {
    const { rows } = await this.db.query(
      `${SELECT_WITH_DERIVED} WHERE gac.company_id = $1 AND gac.status != 'DELETED' ORDER BY gac.sequence ASC, gac.code ASC`,
      [companyId],
    );
  ...
  async filter(companyId: number, filters: Filter[], options?: ListOptions): Promise<GlAccountCategoryRow[]> {
    ...
    const fullWhere = whereSql
      ? `${whereSql} AND gac.company_id = $${params.length + 1} AND gac.status != 'DELETED'`
      : `WHERE gac.company_id = $${params.length + 1} AND gac.status != 'DELETED'`;
  ...
  async search(companyId: number, phrase: string, options?: ListOptions): Promise<GlAccountCategoryRow[]> {
    ...
    params.push(companyId);
    const whereSql = `WHERE (${likeParts.join(" OR ")}) AND gac.company_id = $${params.length} AND gac.status != 'DELETED'`;
    const tail = buildOrderLimitOffset(params, options);
  ...
  async batchGet(companyId: number, codes: string[]): Promise<GlAccountCategoryRow[]> {
    ...
    const { rows } = await this.db.query(
      `${SELECT_WITH_DERIVED} WHERE gac.company_id = $1 AND gac.code = ANY($2::text[]) AND gac.status != 'DELETED' ORDER BY gac.sequence ASC, gac.code ASC`,
      [companyId, codes],
    );
    return rows.map((row: Record<string, unknown>) => this.mapRow(row));
  }
