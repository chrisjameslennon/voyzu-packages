...
import { getStatusSemanticColor } from "@voyzu/modules/common/client";
import { AddInventoryItemModal } from "@voyzu/modules/common/inventory-items/client";
import { Deactivate } from "@voyzu/modules/common/inventory-items/domain/operation-policy";
import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/navigation";

import type { InventoryCategoryResponseDto } from "@voyzu/types/modules/inventory-categories";
import type { InventoryItemCreateRequestDto, InventoryItemResponseDto } from "@voyzu/types/modules/inventory-items";
...
export function OrganizationInventoryItemsListContent({
  items,
  categories,
}: {
  ...
  const transitionStatus = async (action: "activate" | "deactivate") => {
    const response = await fetch(`/api/organization/inventory/items/batch-${action}`, {
      method: "POST",
  ...
  const deleteSelected = async () => {
    const response = await fetch("/api/organization/inventory/items/batch", {
      method: "DELETE",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ codes: selectedRows.map((row) => row.item_code) }),
    });
  ...
  const createItem = async (value: InventoryItemCreateRequestDto): Promise<string | undefined> => {
    const response = await fetch("/api/organization/inventory/items", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    <div className={`${layout.listView} vz-grid-12`}>
      <header className={layout.listHeader}>
        ...
        <div className={layout.slotTitle}>
          ...
          <div className={layout.slotTitleByline}><p className={typography.headingByline}>Default inventory, service, and non-inventory products copied into new companies.</p></div>
        </div>
      ...
      <div className={layout.listBody}>
        <div className={layout.slotBody}>
          <DataTable
            ...
            onRowClick={(row) => router.push(`/organization/inventory/items/${encodeURIComponent(row.item_code)}`)}
            ...
            mobileRender={(row) => <div className={listStyles.mobileCard}><div className={listStyles.mobileCode}>{row.item_code}</div><div className={listStyles.mobileName}><span className={listStyles.mobileNameText}>{row.item_name}</span></div><div className={listStyles.mobileMeta}>{row.category_code} - {row.status}</div></div>}
          />
        </div>
      </div>
