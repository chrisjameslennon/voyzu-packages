...
import { CompanyPageTitleBadges } from "@voyzu/modules/common/client";

import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/navigation";

import type { ArCounterpartySummaryResponseDto } from "@voyzu/types/modules/ar-subledger";
...
import localStyles from "./ar-statements-list.module.css";
...
type StatementRow = ArCounterpartySummaryResponseDto & { id: string };
...
const columns: DataTableColumn<StatementRow>[] = [
  ...
  { key: "openInvoicesAmount", label: "Open Invoices", width: "10rem", align: "right", render: (row) => money(row.openInvoicesAmount) },
  { key: "unappliedReceiptsAmount", label: "Unapplied Receipts", width: "12rem", align: "right", render: (row) => money(row.unappliedReceiptsAmount) },
  { key: "netBalance", label: "Net Balance", width: "10rem", align: "right", render: (row) => money(row.netBalance) },
];

export function ArStatementsListContent({ summaries }: { summaries: ArCounterpartySummaryResponseDto[] }) {
  ...
  const handleExport = async (exportRows: StatementRow[], filename: string) => {
    const response = await fetch("/api/capability/export", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ filename, columns: [{ key: "counterpartyCode", label: "Code" }, { key: "counterpartyName", label: "Counterparty" }, { key: "openInvoicesAmount", label: "Open Invoices" }, { key: "unappliedReceiptsAmount", label: "Unapplied Receipts" }, { key: "netBalance", label: "Net Balance" }], rows: exportRows }) });
  ...
  const exportItems = useMemo<DropdownMenuItem[]>(() => [
    { value: "selected", label: `Selected (${selectedRows.length})`, icon: "check_box", disabled: selectedRows.length === 0, onSelect: () => { void handleExport(selectedRows, "ar_statements_selected"); } },
    { value: "current-view", label: `Current view (${filtered.length})`, icon: "visibility", disabled: filtered.length === 0, onSelect: () => { void handleExport(filtered, "ar_statements_current_view"); } },
    { value: "full-dataset", label: `Full dataset (${rows.length})`, icon: "database", disabled: rows.length === 0, onSelect: () => { void handleExport(rows, "ar_statements_full_dataset"); } },
  ], [filtered, rows, selectedRows]);

  return (
    <div className={`${layout.listView} vz-grid-12`}>
      <header className={layout.listHeader}><div className={layout.slotBreadcrumb}><Breadcrumbs /></div><div className={`${layout.slotTitle} ${localStyles.titleSlot}`}><div className={listStyles.titleIcon}><span className={`material-symbols-outlined ${listStyles.titleIconSymbol}`}>summarize</span></div><h1 className={`${typography.pageTitle} ${layout.pageTitleResponsive}`}>Statements</h1><div className={layout.slotTitleMeta}><CompanyPageTitleBadges /></div><div className={layout.slotTitleByline}><p className={typography.headingByline}>Accounts receivable statements by customer counterparty.</p></div></div><div className={`${layout.slotSearch} ${localStyles.balancesSlot}`}><section className={localStyles.balancesCard} aria-label="Statement totals"><h2 className={localStyles.balancesTitle}>Totals</h2><div className={localStyles.totalRow}><span className={localStyles.balanceLabel}>Total Statements Balance</span><span className={localStyles.balanceValue}>{money(totalBalance)}</span></div></section></div></header>
      ...
      <div className={layout.listBody}><div className={layout.slotBody}><DataTable<StatementRow, string> columns={columns} rows={paginated} selectedIds={selectedIds} isAllSelected={isAllSelected} isSomeSelected={isSomeSelected} onSelectAll={() => setSelectedIds((current) => isAllSelected ? new Set([...current].filter((id) => !paginated.some((row) => row.id === id))) : new Set([...current, ...paginated.map((row) => row.id)]))} onSelectOne={(id) => setSelectedIds((current) => { const next = new Set(current); if (next.has(id)) next.delete(id); else next.add(id); return next; })} onRowClick={(row) => router.push(`/finance/subledgers/ar/statements/${encodeURIComponent(row.counterpartyCode)}`)} currentPage={currentPage} totalPages={totalPages} onPageChange={setCurrentPage} totalCount={rows.length} filteredCount={filtered.length} itemLabel="statements" hasData={rows.length > 0} emptyIcon="summarize" emptyTitle="No statements found" emptyText="No AR counterparties have statement activity" emptyFilterText="No statements match your search" mobileRender={(row) => <div className={listStyles.mobileCard}><div className={listStyles.mobileCode}>{row.counterpartyCode}</div><div className={listStyles.mobileName}><span className={listStyles.mobileNameText}>{row.counterpartyName}</span></div><div className={listStyles.mobileMeta}>Net {money(row.netBalance)}</div></div>} /></div></div>
    </div>
  );
}
