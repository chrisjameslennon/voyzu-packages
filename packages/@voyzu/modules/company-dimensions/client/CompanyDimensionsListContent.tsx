...
import { Deactivate, Delete } from "@voyzu/modules/common/dimensions/domain/operation-policy";
import { AddDimensionModal } from "../../common/dimensions/client";
import { useEffect, useMemo, useState } from "react";
import { useRouter } from "next/navigation";

import type { DimensionCreateRequestDto, DimensionResponseDto } from "@voyzu/types/modules/dimensions";
import {
  Badge,
  Button,
  DataTable,
...
export function CompanyDimensionsListContent({
  dimensions,
  ...
  const canActivateSelection = !readOnly && hasSelection && selectedDimensions.some((dimension) => dimension.status === "INACTIVE");
  const canDeactivateSelection = !readOnly && hasSelection && selectedDimensions.some((dimension) => dimension.status === "ACTIVE") && selectedDimensions.every((dimension) => Deactivate(dimension).length === 0);
  const canDeleteSelection = !readOnly && hasSelection && selectedDimensions.every((dimension) => Delete(dimension).length === 0);
