...
interface Props { account: TaxControlAccountResponseDto; glAccounts: GlAccountResponseDto[]; glAccountId: string; disabled: boolean; onGlAccountChange: (value: string) => void; }

export function OrganizationTaxControlAccountDetailsForm({ account, glAccounts, glAccountId, disabled, onGlAccountChange }: Props) {
  ...
  return <section className={detailStyles.card}><h2 className={typography.sectionHeading}>Control Account Details</h2><div className={detailStyles.formGrid}>
    ...
    <label className={detailStyles.fieldGroup}><span className={`${typography.fieldLabel} ${detailStyles.inlineGroup}`}>GL Account <Badge variant="soft" size="x-small" color="neutral">{account.requiredAccountType}</Badge></span><SearchableSelect value={glAccountId} onChange={onGlAccountChange} options={options} placeholder="Select a GL account" searchPlaceholder="Search GL accounts..." dropdownWidth="auto" disabled={disabled} /></label>
  </div></section>;
}
