...
INSERT INTO inventory_item (company_id, code, name, description, item_type, category_id, unit_code, status, creation_actor_type, updated_actor_type)
SELECT c.id, s.code, s.name, s.description, s.item_type, cat.id, s.unit_code, s.status, 'SYSTEM', 'SYSTEM'
FROM seed s
JOIN company c ON c.code = s.company_code
JOIN inventory_category cat ON cat.company_id = c.id AND cat.code = s.category_code
