...
INSERT INTO tax_control_account (company_id, code, ledger, name, description, tax_family_code, status, gl_account_id, creation_actor_type, updated_actor_type)
SELECT c.id, s.code, s.ledger, s.name, s.description, s.tax_family_code, s.status, ga.id, 'SYSTEM', 'SYSTEM'
FROM seed s
