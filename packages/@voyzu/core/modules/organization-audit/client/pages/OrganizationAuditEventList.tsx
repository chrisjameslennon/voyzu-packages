...
interface FinancialYearResponseDto {
  code: string;
  startDate: string;
  endDate: string;
}
...
export function OrganizationAuditEventList({
  ...
  useEffect(() => {
    ...
    const params = new URLSearchParams({ packageCode: "@voyzu/core" });
    fetch(`${API_BASE_PATH}/count?${params.toString()}`, { signal: controller.signal })
      .then((r) => r.json())
      .then((response: AuditEventCountResponseDto) => setTotalDbCount(response.count))
      .catch(() => { /* ignore abort */ });
