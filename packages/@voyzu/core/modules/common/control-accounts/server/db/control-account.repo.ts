...
const COLUMNS: readonly string[] = [
  "finance_company_id", "code", "ledger", "name", "gl_account_id", "status",
...
const CONTROL_ACCOUNT_UNION_SQL = `
  (
    SELECT finance_company_id, ledger, code, name, gl_account_id, status,
           creation_date, creation_actor_type, creation_user_id, creation_mutation_id,
           updated_date, updated_actor_type, updated_user_id, updated_mutation_id
...
const JOIN_SQL = `LEFT JOIN gl_account ga ON ga.finance_company_id = ca.finance_company_id AND ga.id = ca.gl_account_id`;
const COMPANIES_WITH_POSTINGS_SQL = `COALESCE(ARRAY(
  ...
  WHERE source_finance_company.id = ca.finance_company_id
...
export class ControlAccountRepo {
  ...
  async get(companyId: number, code: string): Promise<ControlAccountRow | null> {
    const { rows } = await this.db.query(
      `${SELECT_SQL} FROM ${CONTROL_ACCOUNT_UNION_SQL} ca ${JOIN_SQL} WHERE ca.finance_company_id = $1 AND ca.code = $2`,
      [companyId, code],
    );
  ...
  async listAll(companyId: number): Promise<ControlAccountRow[]> {
    const { rows } = await this.db.query(
      `${SELECT_SQL} FROM ${CONTROL_ACCOUNT_UNION_SQL} ca ${JOIN_SQL} WHERE ca.finance_company_id = $1 ORDER BY ca.code ASC`,
      [companyId],
    );
  ...
  async listByCodes(companyId: number, codes: readonly string[]): Promise<ControlAccountRow[]> {
    ...
    const { rows } = await this.db.query(
      `${SELECT_SQL} FROM ${CONTROL_ACCOUNT_UNION_SQL} ca ${JOIN_SQL} WHERE ca.finance_company_id = $1 AND ca.code = ANY($2::text[])`,
      [companyId, [...codes]],
    );
    return rows.map((r: Record<string, unknown>) => this.mapRow(r));
  }
  ...
  async filter(companyId: number, filters: Filter[], options?: ListOptions): Promise<ControlAccountRow[]> {
    ...
    const scopedWhere = shiftedWhere ? `WHERE ca.finance_company_id = $1 AND ${shiftedWhere.slice("WHERE ".length)}` : "WHERE ca.finance_company_id = $1";
  ...
  async search(companyId: number, phrase: string, options?: ListOptions): Promise<ControlAccountRow[]> {
    ...
    const whereSql = `WHERE ca.finance_company_id = $1 AND (${likeParts.join(" OR ")})`;
