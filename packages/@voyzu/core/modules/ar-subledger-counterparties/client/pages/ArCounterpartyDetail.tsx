...
import { ArCounterpartyReportTemplate } from "../templates/ArCounterpartyReportTemplate";
import localStyles from "./ar-counterparty-detail.module.css";
...
export function ArCounterpartyDetail({
  company,
  counterparty,
}: {
  ...
  const printablePath = `/finance/subledgers/ar/counterparties/${encodeURIComponent(counterparty.code)}/printable`;
  const pdfParams = new URLSearchParams({
    orientation: "portrait",
    path: printablePath,
    filename: `ar-counterparty-${counterparty.code}`,
  });
        ...
        <div className={localStyles.reportTab}>
          ...
          <div className={localStyles.documentShell}>
            <div className={`${reportLayout.document} ${localStyles.portraitDocument}`}>
              <ArCounterpartyReportTemplate company={company} counterparty={counterparty} generatedAt={generatedAt} />
            </div>
          </div>
        </div>
      ),
    },
    {
      ...
      content: (
        <section className={detailStyles.card}>
          <h2 className={typography.sectionHeading}>AR Counterparty</h2>
          <div className={detailStyles.formGrid}>
            {fields.map((field) => (
              <label key={field.label} className={detailStyles.fieldGroup}>
                <span className={typography.fieldLabel}>{field.label}</span>
  ...
  return (
    <div className={`${layout.detailView} ${layout.detailViewWithStatusRail}`}>
      <header className={layout.detailHeader}>
        ...
        <div className={layout.slotActions}><div className={detailStyles.headerActions}><DetailBackButton fallbackHref="/finance/subledgers/ar/counterparties" /></div></div>
      </header>
