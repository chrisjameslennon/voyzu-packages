...
export class ArInvoicePostingRepo {
  ...
  async getCompanyByCode(code: string): Promise<CompanyPostingContextRow | null> {
    const { rows } = await this.db.query(
      `SELECT id, code, name, country_code, base_currency_code, status
       FROM company
       WHERE code = $1`,
      [code],
    );
  ...
  async listItemPostingProfiles(companyId: number, itemCodes: string[]): Promise<ArInvoiceItemPostingProfileRow[]> {
    ...
    const { rows } = await this.db.query(
      `SELECT ii.code AS item_code, ii.item_type, ii.status AS item_status,
       ...
       LEFT JOIN gl_account ga ON ga.company_id = ipp.company_id AND ga.id = ipp.revenue_gl_account_id
       WHERE ii.company_id = $1 AND ii.code = ANY($2::text[])`,
      [companyId, itemCodes],
    );
    return rows.map((row: Record<string, unknown>) => ({
      item_code: String(row.item_code),
      item_type: row.item_type as ArInvoiceItemPostingProfileRow["item_type"],
      item_status: row.item_status as ArInvoiceItemPostingProfileRow["item_status"],
  ...
  async insertArSubledgerEntry(
    ...
    const { rows } = await this.db.query(
      `INSERT INTO ar_subledger_entry_header
         (code, company_id, journal_header_id, ar_counterparty_id,
