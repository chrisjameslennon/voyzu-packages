...
This is an internal development pattern for `@voyzu/core`. The company switcher belongs to Core rather than the Voyzu platform because company selection is part of Core's Finance domain.
...
```tsx
import { resolveServerCompanyApiContext } from "@voyzu/core/common/server";
