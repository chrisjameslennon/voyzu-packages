...
const ACTIVE_CODES = new Set(["AUD", "CAD", "GBP", "NZD", "USD"]);
...
async function main() {
  ...
  if (reset) {
    ...
    await pool.query("DELETE FROM currency");
    await pool.query("SET session_replication_role = DEFAULT");
    console.log("Reset currency.");
  }

  for (const c of currencies) {
    const status = ACTIVE_CODES.has(c.code) ? "ACTIVE" : "INACTIVE";
    await pool.query(
      ...
      [c.code, c.name, c.symbol, status]
    );
  }

  console.log(`seeded ${currencies.length} currencies (${ACTIVE_CODES.size} active)`);
  await pool.end();
}
