DROP TABLE IF EXISTS ap_subledger_entry_line CASCADE;
DROP TABLE IF EXISTS ap_subledger_entry_header CASCADE;
DROP TABLE IF EXISTS ap_subledger_entry CASCADE;

CREATE TABLE ap_subledger_entry_header (
    id                          BIGINT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY (START WITH 10000),
    code                        TEXT NOT NULL UNIQUE,
    company_id                  BIGINT NOT NULL,
    journal_header_id           BIGINT NOT NULL,
    ap_counterparty_id          BIGINT NOT NULL,
    document_type_code          business_code NOT NULL,
    document_id                 TEXT NOT NULL,
    supplier_invoice_number     TEXT,
    description                 TEXT NOT NULL,
    memo                        TEXT,
    document_date               DATE NOT NULL,
    posting_date                DATE NOT NULL,
    financial_year_id           BIGINT NOT NULL,
    financial_period_id         BIGINT NOT NULL,
    base_currency_code          iso_currency_code NOT NULL,
    status                      TEXT NOT NULL DEFAULT 'POSTED' CHECK (status = 'POSTED'),

    creation_date               audit_timestamp,
    creation_actor_type         actor_type,
    creation_user_id            TEXT,
    creation_mutation_id UUID,

    updated_date                audit_timestamp,
    updated_actor_type          actor_type,
    updated_user_id             TEXT,
    updated_mutation_id UUID,

    deletion_date audit_timestamp,
    deletion_actor_type actor_type,
    deletion_user_id TEXT,
    deletion_mutation_id UUID,

    CONSTRAINT fk_ap_header_company          FOREIGN KEY (company_id)           REFERENCES company(id),
    CONSTRAINT fk_ap_header_journal          FOREIGN KEY (journal_header_id)    REFERENCES journal_header(id),
    CONSTRAINT fk_ap_header_counterparty     FOREIGN KEY (ap_counterparty_id)   REFERENCES ap_counterparty(id),
    CONSTRAINT fk_ap_header_document_type    FOREIGN KEY (document_type_code) REFERENCES financial_document_type(code),
    CONSTRAINT fk_ap_header_financial_year   FOREIGN KEY (financial_year_id)    REFERENCES fiscal_year(id),
    CONSTRAINT fk_ap_header_financial_period FOREIGN KEY (financial_period_id)  REFERENCES fiscal_period(id),
    CONSTRAINT fk_ap_header_base_currency    FOREIGN KEY (base_currency_code)   REFERENCES currency(code)
);
